//! MCP dispatch stub for the `gotify` tool.
//!
...
/// Dispatch one MCP call against the gotify tool.
///
/// # Errors
...
pub async fn dispatch(action: &str, _params: Value) -> Result<Value> {
    match action {
        "help" => Ok(serde_json::json!({
            "service": "gotify",
            "message": "gotify is not yet implemented",
            "actions": []
        })),
        _ => anyhow::bail!("gotify is not yet implemented — action: {action}"),
    }
}
