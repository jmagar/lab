...
pub const ACTIONS: &[ActionSpec] = &[
    ...
    ActionSpec {
        ...
        description: "List pending, approved, and denied device enrollments",
        destructive: false,
        returns: "Value",
        params: &[],
    },
    ActionSpec {
        ...
        params: &[
            ParamSpec {
                ...
                ty: "string",
                required: true,
                description: "Device ID to approve",
            },
            ParamSpec {
                name: "note",
                ty: "string",
    ...
    ActionSpec {
        ...
        description: "Deny a pending or approved device enrollment",
        destructive: true,
        returns: "Value",
        params: &[
            ParamSpec {
...
pub async fn dispatch(action: &str, params: Value) -> Result<Value, ToolError> {
    let store = EnrollmentStore::open(crate::device::enrollment::store::default_store_path())
        .await
    ...
    match action {
        ...
        "enrollments.approve" => {
            let device_id = params
                ...
                .and_then(Value::as_str)
                .ok_or_else(|| ToolError::MissingParam {
                    message: "missing required param `device_id`".to_string(),
                    param: "device_id".to_string(),
                })?;
            let note = params.get("note").and_then(Value::as_str).map(str::to_string);
            let approved = store.approve(device_id, note).await.map_err(|error| ToolError::Sdk {
        ...
        "enrollments.deny" => {
            ...
            let reason = params
                ...
                .and_then(Value::as_str)
                .map(str::to_string);
            let denied = store.deny(device_id, reason).await.map_err(|error| ToolError::Sdk {
                sdk_kind: "not_found".to_string(),
                message: error.to_string(),
            })?;
...
mod tests {
    use super::*;
    use crate::device::enrollment::store::{EnrollmentAttempt, TailnetIdentity};

    #[tokio::test]
    async fn device_mcp_dispatch_supports_enrollment_actions() {
        let path = crate::device::enrollment::store::default_store_path();
