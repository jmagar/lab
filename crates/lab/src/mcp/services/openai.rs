//! MCP dispatch stub for the `openai` tool.
//!
...
/// Dispatch one MCP call against the openai tool.
///
/// # Errors
...
pub async fn dispatch(action: &str, _params: Value) -> Result<Value> {
    match action {
        "help" => Ok(serde_json::json!({
            "service": "openai",
            "message": "openai is not yet implemented",
            "actions": []
        })),
        _ => anyhow::bail!("openai is not yet implemented — action: {action}"),
    }
}
