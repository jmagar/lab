//! MCP dispatch stub for the `plex` tool.
//!
...
/// Dispatch one MCP call against the plex tool.
///
/// # Errors
...
pub async fn dispatch(action: &str, _params: Value) -> Result<Value> {
    match action {
        "help" => Ok(serde_json::json!({
            "service": "plex",
            "message": "plex is not yet implemented",
            "actions": []
        })),
        _ => anyhow::bail!("plex is not yet implemented — action: {action}"),
    }
}
