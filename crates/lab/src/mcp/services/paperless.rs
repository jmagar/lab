//! MCP dispatch stub for the `paperless` tool.
//!
...
/// Dispatch one MCP call against the paperless tool.
///
/// # Errors
...
pub async fn dispatch(action: &str, _params: Value) -> Result<Value> {
    match action {
        "help" => Ok(serde_json::json!({
            "service": "paperless",
            "message": "paperless is not yet implemented",
            "actions": []
        })),
        _ => anyhow::bail!("paperless is not yet implemented — action: {action}"),
    }
}
