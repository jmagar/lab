//! MCP dispatch stub for the `tautulli` tool.
//!
...
/// Dispatch one MCP call against the tautulli tool.
///
/// # Errors
...
pub async fn dispatch(action: &str, _params: Value) -> Result<Value> {
    match action {
        "help" => Ok(serde_json::json!({
            "service": "tautulli",
            "message": "tautulli is not yet implemented",
            "actions": []
        })),
        _ => anyhow::bail!("tautulli is not yet implemented — action: {action}"),
    }
}
