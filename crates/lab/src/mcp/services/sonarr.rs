//! MCP dispatch stub for the `sonarr` tool.
//!
...
/// Dispatch one MCP call against the sonarr tool.
///
/// # Errors
...
pub async fn dispatch(action: &str, _params: Value) -> Result<Value> {
    match action {
        "help" => Ok(serde_json::json!({
            "service": "sonarr",
            "message": "sonarr is not yet implemented",
            "actions": []
        })),
        _ => anyhow::bail!("sonarr is not yet implemented — action: {action}"),
    }
}
