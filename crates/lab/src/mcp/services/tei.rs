//! MCP dispatch stub for the `tei` tool.
//!
...
/// Dispatch one MCP call against the tei tool.
///
/// # Errors
...
pub async fn dispatch(action: &str, _params: Value) -> Result<Value> {
    match action {
        "help" => Ok(serde_json::json!({
            "service": "tei",
            "message": "tei is not yet implemented",
            "actions": []
        })),
        _ => anyhow::bail!("tei is not yet implemented — action: {action}"),
    }
}
