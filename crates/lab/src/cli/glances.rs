...
/// Run the `lab glances` subcommand.
pub async fn run(args: GlancesArgs, format: OutputFormat) -> Result<ExitCode> {
    ...
    run_confirmable_action_command(
        ...
        args.yes,
        format,
        |action, params| async move { crate::dispatch::glances::dispatch(&action, params).await },
    )
    .await
}
