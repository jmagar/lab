...
use crate::dispatch::immich::ACTIONS;
...
/// Run the `lab immich` subcommand.
pub async fn run(args: ImmichArgs, format: OutputFormat) -> Result<ExitCode> {
    ...
    run_confirmable_action_command(
        ...
        args.yes,
        format,
        |action, params| async move { crate::dispatch::immich::dispatch(&action, params).await },
    )
    .await
}
