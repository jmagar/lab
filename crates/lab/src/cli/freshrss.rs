...
use crate::dispatch::freshrss::ACTIONS;
...
/// Run the `lab freshrss` subcommand.
pub async fn run(args: FreshrssArgs, format: OutputFormat) -> Result<ExitCode> {
    ...
    run_confirmable_action_command(
        ...
        args.yes,
        format,
        |action, params| async move { crate::dispatch::freshrss::dispatch(&action, params).await },
    )
    .await
}
