//! `lab tautulli` — thin CLI shim for the Tautulli service.
//!
...
/// Run the `lab tautulli` subcommand.
///
/// # Errors
...
pub async fn run(args: TautulliArgs, format: OutputFormat) -> Result<ExitCode> {
    ...
    if args.dry_run {
        print_dry_run("tautulli", &args.action, &params);
    ...
    run_confirmable_action_command(
        ...
        args.yes,
        format,
        |action, params| async move { crate::dispatch::tautulli::dispatch(&action, params).await },
    )
    .await
}
