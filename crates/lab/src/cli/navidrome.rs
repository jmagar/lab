...
use crate::dispatch::navidrome::ACTIONS;
...
/// Run the `lab navidrome` subcommand.
pub async fn run(args: NavidromeArgs, format: OutputFormat) -> Result<ExitCode> {
    ...
    run_confirmable_action_command(
        ...
        args.yes,
        format,
        |action, params| async move { crate::dispatch::navidrome::dispatch(&action, params).await },
    )
    .await
}
