...
/// Run the `lab scrutiny` subcommand.
pub async fn run(args: ScrutinyArgs, format: OutputFormat) -> Result<ExitCode> {
    ...
    run_confirmable_action_command(
        ...
        args.yes,
        format,
        |action, params| async move { crate::dispatch::scrutiny::dispatch(&action, params).await },
    )
    .await
}
