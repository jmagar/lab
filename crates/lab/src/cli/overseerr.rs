//! `lab overseerr` — thin CLI shim for the Overseerr service.
//!
...
/// Run the `lab overseerr` subcommand.
///
/// # Errors
/// Returns an error if dispatch fails.
pub async fn run(args: OverseerrArgs, format: OutputFormat) -> Result<ExitCode> {
    let params = args
        .params
        .as_deref()
    ...
    if args.dry_run {
        print_dry_run("overseerr", &args.action, &params);
