//! `lab paperless` — thin CLI shim for the Paperless service.
//!
...
/// Run the `lab paperless` subcommand.
///
/// # Errors
/// Returns an error if dispatch fails.
pub async fn run(args: PaperlessArgs, format: OutputFormat) -> Result<ExitCode> {
    let params = args
        .params
        .as_deref()
    ...
    if args.dry_run {
        print_dry_run("paperless", &args.action, &params);
