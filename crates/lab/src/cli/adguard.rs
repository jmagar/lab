...
use crate::dispatch::adguard::ACTIONS;
...
/// Run the `lab adguard` subcommand.
pub async fn run(args: AdguardArgs, format: OutputFormat) -> Result<ExitCode> {
    ...
    run_confirmable_action_command(
        ...
        args.yes,
        format,
        |action, params| async move { crate::dispatch::adguard::dispatch(&action, params).await },
    )
    .await
}
