...
pub enum DeviceCommand {
    ...
    List,
    /// Get details for a specific device by `node_id`.
    Get { node_id: String },
...
pub async fn run(args: DeviceArgs, format: OutputFormat, config: &LabConfig) -> Result<ExitCode> {
    match args.command {
        ...
        DeviceCommand::Get { node_id } => {
            print(&fetch_device(config, &node_id).await?, format)?;
        }
...
pub async fn fetch_device(config: &LabConfig, node_id: &str) -> Result<serde_json::Value> {
    MasterClient::from_config(config, None)?
