//! Shared dispatch layer for the `Memos` service.
//!
//! Feature-gated: only compiled when the `memos` feature is enabled.
//!
//! This module is the single authoritative owner of the Memos action
...
mod tests {
    ...
    fn catalog_includes_core_actions() {
        ...
        assert!(names.contains(&"memos.list"));
        assert!(names.contains(&"memos.get"));
        assert!(names.contains(&"memos.create"));
        assert!(names.contains(&"memos.update"));
        assert!(names.contains(&"memos.delete"));
        assert!(names.contains(&"tags.list"));
        assert!(names.contains(&"workspace.profile"));
        assert!(names.contains(&"user.me"));
    ...
    fn non_destructive_actions_are_not_marked() {
        ...
        assert!(
            !create.destructive,
            "memos.create must not be marked destructive"
        );
    }
    ...
    fn client_from_vars_accepts_valid() {
        assert!(
            client::client_from_vars(Some("http://localhost:5230"), Some("mytoken123")).is_some()
        );
    }
}
