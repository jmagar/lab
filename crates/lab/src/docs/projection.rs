...
use super::types::{
    ActionDoc, DocsProjection, EnvDoc, FeatureClass, FeatureDoc, FeatureMatrix, FeatureMismatch,
    ParamDoc, ServiceDoc, ServiceExposure, SurfaceAvailability,
};
...
pub fn build_docs_projection(repo_root: &Path) -> Result<DocsProjection> {
    ...
    let action_catalog = build_action_catalog(services);
...
fn build_action_catalog(services: &[RegisteredService]) -> Vec<ActionDoc> {
...
fn service_surfaces(service: &str) -> SurfaceAvailability {
    SurfaceAvailability {
