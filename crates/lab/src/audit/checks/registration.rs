...
pub fn run(name: &str, shared: &SharedContext, repo_root: &Path) -> Vec<(String, CheckResult)> {
    ...
    let mcp_services = shared.get(repo_root, "crates/lab/src/mcp/services.rs").unwrap_or("");
    let registry = shared.get(repo_root, "crates/lab/src/mcp/registry.rs").unwrap_or("");
    let api_services = shared.get(repo_root, "crates/lab/src/api/services.rs").unwrap_or("");
    let router = shared.get(repo_root, "crates/lab/src/api/router.rs").unwrap_or("");
    let state = shared.get(repo_root, "crates/lab/src/api/state.rs").unwrap_or("");
    let tui = shared.get(repo_root, "crates/lab/src/tui/metadata.rs").unwrap_or("");

    out.push(("feature.lab-apis".into(), contains_check(lab_apis_cargo, &feature)));
    out.push(("feature.lab".into(), contains_check(lab_cargo, &format!("{name} = [\"lab-apis/{name}\"]"))));
