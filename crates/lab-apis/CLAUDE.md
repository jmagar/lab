...
HTTP client library for 22 feature-gated homelab services, plus always-on `extract` and `device_runtime`. No binary dependencies (`clap`, `rmcp`,
...
22 opt-in features. `core`, `extract`, and `device_runtime` are always compiled — no gate possible.
