...
Then simply give the `reverse\_proxy` directive a path matcher. The path should be the same as the `Base URL` you entered into Jellyfin's settings. The following example is for a server that is accessible at `example.com/jellyfin`.
```
`
example.com
redir /jellyfin /jellyfin/
reverse\_proxy /jellyfin/\* 127.0.0.1:8096
`
```
