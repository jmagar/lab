...
## Using Tailscale Directly[​](#using-tailscale-directly)
...
## Using Tailscale with a reverse proxy[​](#using-tailscale-with-a-reverse-proxy)
...
3. Open the Dashboard of the Jellyfin server and go to Networking. Add the reverse proxy Tailscale IP to the `known proxies` setting. This is explained in detail in our [reverse proxy guide](/docs/general/post-install/networking/reverse-proxy/#forwarded-for-headers).
...
3. Setup a reverse proxy of your choice and set the upstream server to the IP from above. Example [Caddy](/docs/general/post-install/networking/reverse-proxy/caddy) configuration: (Assuming server has Tailscale IP of 100.12.34.56)
```
`
example.com
redir /jellyfin /jellyfin/
reverse\_proxy /jellyfin/\* 100.12.34.56:8096
`
```
