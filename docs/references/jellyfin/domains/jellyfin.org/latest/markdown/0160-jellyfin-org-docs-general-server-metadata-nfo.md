...
## Reading metadata from .nfo files[​](#reading-metadata-from-nfo-files)
...
Provider id tags are supported as well if they follow the scheme: `\<PROVIDER\_NAME`+ `id\>`.
...
## Writing metadata to .nfo files[​](#writing-metadata-to-nfo-files)
...
|airsafter\_season|only for tv show episode|
|airsbefore\_episode|only for tv show episode|
|airsbefore\_season|only for tv show episode|
...
|trailer|YouTube URL in the Kodi format `plugin://plugin.video.youtube/?action=play\_video&videoid=\<Youtube Video ID\>`|
|rating||
|year||
|sorttitle||
|mpaa||
...
## Image paths and URLs in .nfo files[​](#image-paths-and-urls-in-nfo-files)
