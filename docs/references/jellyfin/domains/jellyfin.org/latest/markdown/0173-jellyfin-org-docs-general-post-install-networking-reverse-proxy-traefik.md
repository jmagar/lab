...
user: 1000:1000
group\_add: # by id as these may not exist within the container. Needed to provide permissions to the VAAPI Devices
- '107' #render
- '44' #video
...
- 'traefik.http.routers.jellyfin.rule=Host(`HOST\_NAME.DOMAIN\_NAME`)' # OPTIONAL: && PathPrefix(`/jellyfin`)
#### Enable TLS with the ACME/LetsEncrypt resolver for HOSTNAME.DOMAIN\_NAME
- 'traefik.http.routers.jellyfin.tls=true'
- 'traefik.http.routers.jellyfin.tls.certResolver=leresolver'
- 'traefik.http.routers.jellyfin.tls.domains=HOSTNAME.DOMAIN\_NAME'
## Middleware
...
- 'traefik.http.middlewares.jellyfin-mw.headers.SSLHost=HOST\_NAME.DOMAIN\_NAME:9999'
...
- "traefik.http.middlewares.jellyfin-mw.headers.customFrameOptionsValue='allow-from https://DOMAIN\_NAME'"
## HTTP Service
...
- 'traefik.http.routers.jellyfin-insecure.rule=Host(`HOST\_NAME.DOMAIN\_NAME`)' # OPTIONAL: && PathPrefix(`/jellyfin`)
...
`
RFC2136\_NAMESERVER=...
RFC2136\_TSIG\_ALGORITHM=hmac-sha512.
RFC2136\_TSIG\_KEY=...
RFC2136\_TSIG\_SECRET=...
`
```
...
```
caution
These configurations use DOMAIN\_NAME (i.e.: example.com) and HOST\_NAME (i.e.: servername) throughout it. You should replace these with your server's name. HOST\_NAME.DOMAIN\_NAME refers to the machine itself (ie: servername.example.com). Don't forget to update `traefik.toml`'s YOU@DOMAIN\_NAME with your email address. Let's Encrypt does not require a valid email but invalid e-mails may be flagged as fake.
