...
## Running Jellyfin Behind a Reverse Proxy[​](#running-jellyfin-behind-a-reverse-proxy)
Important things to note when using Jellyfin behind a reverse proxy.
### Logging[​](#logging)
Be careful when logging requests with your reverse proxy. Jellyfin sometimes sends authentication information as part of the URL (e.g. `api\_key` parameter), so logging the full request path can expose secrets to your logfile.
...
Our [proxy guides](/docs/general/post-install/networking/reverse-proxy/#guides) include examples on how to censor sensitive information from a logfile.
...
## Guides[​](#guides)
We recommend using [Caddy](https://caddyserver.com/) for its ease of use, especially with https. We provide a [guide](/docs/general/post-install/networking/reverse-proxy/caddy/) for configuring Caddy with Jellyfin.
...
* [Nginx](/docs/general/post-install/networking/reverse-proxy/nginx)
* [Traefik](/docs/general/post-install/networking/reverse-proxy/traefik)
* [HAProxy](/docs/general/post-install/networking/reverse-proxy/haproxy)
* [Apache](/docs/general/post-install/networking/reverse-proxy/apache)
While not a reverse proxy, Let's Encrypt can be used independently or with a reverse proxy to provide SSL certificates.
* [Let's Encrypt](/docs/general/post-install/networking/advanced/letsencrypt)
When following these guides, be sure to replace the following variables with your information.
* `DOMAIN\_NAME`: Your public domain name to access Jellyfin on (e.g. jellyfin.example.com)
* `example.com`: The domain name Jellyfin services will run under (e.g. example.com)
* `SERVER\_IP\_ADDRESS`: The IP address of your Jellyfin server (if the reverse proxy is on the same server use 127.0.0.1)
In addition, the examples are configured for use with Let's Encrypt certificates. If you have a certificate from another source, change the SSL configuration from `/etc/letsencrypt/DOMAIN\_NAME/` to the location of your certificate and key.
