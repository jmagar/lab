...
## Raspberry Pi (Remote Controlled Speakers)[​](#raspberry-pi-remote-controlled-speakers)
...
album\_format = {ProductionYear} - {Name} (Optional: will default to "{Name}" if left undefined)
`
```
...
* `album\_format` can be used to change the display format of music albums when using the file browser view. Currently the only really usable fields are ProductionYear and Name
