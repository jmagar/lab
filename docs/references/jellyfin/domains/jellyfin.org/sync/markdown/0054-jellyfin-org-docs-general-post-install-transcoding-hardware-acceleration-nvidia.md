...
Please refer to [this section](/docs/general/post-install/transcoding/hardware-acceleration/known-issues#nvidia) for known issues and limitations.
note
...
### Configure With Windows Virtualization[​](#configure-with-windows-virtualization)
NVIDIA Windows driver provides access to the NVENC/NVDEC and CUDA in Windows WSL2 and Docker.
Refer to [Configure On Linux Host](/docs/general/post-install/transcoding/hardware-acceleration/nvidia#configure-on-linux-host) and [Configure With Linux Virtualization](/docs/general/post-install/transcoding/hardware-acceleration/nvidia#configure-with-linux-virtualization).
...
* Use the Docker command line **or** docker-compose:
...
```
note
If you encounter the upstream issue `CUDA\_ERROR\_NO\_DEVICE: no CUDA-capable device is detected`, pass these extra devices to the Docker:
```
`
...
* `NVIDIA\_DRIVER\_CAPABILITIES=all`
...
b. Debian, Ubuntu
`echo "deb https://developer.download.nvidia.com/compute/cuda/repos/ubuntu2404/x86\_64/cuda-ubuntu2404.pin stable main" | sudo tee /etc/apt/sources.list.d/cuda-ubuntu2404.list`
`sudo apt-get update`
...
--user $(id -u):$(id -g) \\
--userns keep-id \\
--environment=JELLYFIN\_DATA\_DIR=/var/lib/jellyfin
--environment=JELLYFIN\_CONFIG\_DIR=/etc/jellyfin
--environment=JELLYFIN\_LOG\_DIR=/var/log/jellyfin
--environment=JELLYFIN\_CACHE\_DIR=/var/cache/jellyfin
