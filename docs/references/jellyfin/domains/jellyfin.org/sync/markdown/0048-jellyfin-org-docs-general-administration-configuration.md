...
2. Environment variable `JELLYFIN\_DATA\_DIR`, if specified
...
5. `$XDG\_DATA\_HOME/jellyfin`, if `$XDG\_DATA\_HOME` exists
6. `$HOME/.local/share/jellyfin`
note
Windows users can also specify the data directory using a Windows Registry string key called `DataFolder` located at `Computer\\HKEY\_LOCAL\_MACHINE\\SOFTWARE\\WOW6432Node\\Jellyfin\\Server`
An additional string key called `InstallFolder` in `Computer\\HKEY\_LOCAL\_MACHINE\\SOFTWARE\\WOW6432Node\\Jellyfin\\Server` can also specify the install location.
...
2. Environment variable `JELLYFIN\_CONFIG\_DIR`, if specified
3. `\<Data Directory\>/config`, if it exists or if running on Windows
4. `$XDG\_CONFIG\_HOME/jellyfin` if `$XDG\_CONFIG\_HOME` exists
...
2. Environment variable `$JELLYFIN\_CACHE\_DIR`, if specified
3. `\<Data Directory\>/cache`, if Windows
4. `$XDG\_CACHE\_HOME/jellyfin` if `$XDG\_CACHE\_HOME` exists
...
2. Environment variable `$JELLYFIN\_WEB\_DIR`, if specified
...
2. Environment variable `$JELLYFIN\_LOG\_DIR`, if specified
...
4. **Environment variables**: The [documentation](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/configuration/?view=aspnetcore-8.0#non-prefixed-environment-variables) provided by Microsoft explains how to set these configuration options via environment variables. Jellyfin uses its own custom `JELLYFIN\_` prefix for these variables. For example, to set a value for the `HttpListenerHost:DefaultRedirectPath` setting, you would set a value for the `JELLYFIN\_HttpListenerHost\_\_DefaultRedirectPath` environment variable.
...
### Server Side System Fonts[​](#server-side-system-fonts)
...
### Client Side System Fonts[​](#client-side-system-fonts)
