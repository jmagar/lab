...
Learn more about reverse proxies in our dedicated [Reverse Proxy guide](/docs/general/post-install/networking/reverse-proxy/).
### SSL / https[​](#ssl--https)
...
Please use a trusted certificate authority such as [Let's Encrypt](/docs/general/post-install/networking/advanced/letsencrypt) when using https.
caution
Self-signed certificates pose security and compatibility issues and are strongly discouraged.
While Jellyfin supports HTTPS, it is strongly recommended to handle HTTPS termination separately on a reverse proxy. You can find more info on how to set this up on our [Reverse Proxy](/docs/general/post-install/networking/reverse-proxy/) page.
...
caution
Base URL is known to break HDHomeRun, the [DLNA plugin](/docs/general/post-install/networking/dlna), Sonarr, Radarr, and MrMC.
...
2. Client applications generally, for now, do not handle the Base URL redirects implicitly. Therefore, for instance in the Android TV app, the `Host` setting *must* include the BaseURL as well (e.g. `http://myserver:8096/baseurl`), or the connection will fail.
