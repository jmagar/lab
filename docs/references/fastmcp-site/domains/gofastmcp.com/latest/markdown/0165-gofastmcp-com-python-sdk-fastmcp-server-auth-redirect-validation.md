...
`fastmcp.server.auth.redirect\_validation`
...
`matches\_allowed\_pattern` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/server/auth/redirect_validation.py#L132)
```
`matches\_allowed\_pattern(uri: str, pattern: str) -\> bool
`
```
...
`validate\_redirect\_uri` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/server/auth/redirect_validation.py#L187)
```
`validate\_redirect\_uri(redirect\_uri: str | AnyUrl | None, allowed\_patterns: list[str] | None) -\> bool
`
```
Validate a redirect URI against allowed patterns.
**Args:**
* `redirect\_uri`: The redirect URI to validate
* `allowed\_patterns`: List of allowed patterns. If None, all URIs are allowed (for DCR compatibility).
