...
app = FastAPI()
app.mount("/mcp", mcp.http\_app()) # Session manager won't initialize
`
```
...
@asynccontextmanager
async def combined\_lifespan(app: FastAPI):
# Run both lifespans
async with app\_lifespan(app):
async with mcp\_app.lifespan(app):
yield
# Use the combined lifespan
app = FastAPI(lifespan=combined\_lifespan)
app.mount("/mcp", mcp\_app)
`
```
