...
`install\_stdio` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/cli/install/stdio.py#L21)
```
`install\_stdio(file: Path, server\_object: str | None) -\> bool
`
```
...
`stdio\_command` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/cli/install/stdio.py#L78)
```
`stdio\_command(server\_spec: str) -\> None
`
```
Generate the stdio command for running a FastMCP server.
...
**Args:**
* `server\_spec`: Python file to run, optionally with :object suffix