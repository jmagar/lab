...
`ClientToolsMixin` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/client/mixins/tools.py#L34)
Mixin providing tool-related methods for Client.
**Methods:**
####
[​
](#list_tools_mcp)
`list\_tools\_mcp` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/client/mixins/tools.py#L39)
```
`list\_tools\_mcp(self: Client) -\> mcp.types.ListToolsResult
`
```
...
`list\_tools` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/client/mixins/tools.py#L62)
```
`list\_tools(self: Client, max\_pages: int = AUTO\_PAGINATION\_MAX\_PAGES) -\> list[mcp.types.Tool]
`
```
...
`call\_tool\_mcp` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/client/mixins/tools.py#L111)
```
`call\_tool\_mcp(self: Client, name: str, arguments: dict[str, Any], progress\_handler: ProgressHandler | None = None, timeout: datetime.timedelta | float | int | None = None, meta: dict[str, Any] | None = None) -\> mcp.types.CallToolResult
`
```
...
* `progress\_handler`: The progress handler to use for the tool call. Defaults to None.
...
can access this via `context.request\_context.meta`. Defaults to None.
**Returns:**
...
`call\_tool` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/client/mixins/tools.py#L191)
```
`call\_tool(self: Client, name: str, arguments: dict[str, Any] | None = None) -\> CallToolResult
`
```
####
[​
](#call_tool-2)
`call\_tool` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/client/mixins/tools.py#L205)
```
`call\_tool(self: Client, name: str, arguments: dict[str, Any] | None = None) -\> ToolTask
`
```
####
[​
](#call_tool-3)
`call\_tool` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/client/mixins/tools.py#L220)
```
`call\_tool(self: Client, name: str, arguments: dict[str, Any] | None = None) -\> CallToolResult | ToolTask
`
```
...
* `timeout`: The timeout for the tool call. Defaults to None.
* `progress\_handler`: The progress handler to use for the tool call. Defaults to None.
* `raise\_on\_error`: Whether to raise an exception if the tool call results in an error. Defaults to True.
* `meta`: Additional metadata to include with the request.
This is useful for passing contextual information (like user IDs, trace IDs, or preferences)
that shouldn’t be tool arguments but may influence server-side processing. The server
can access this via `context.request\_context.meta`. Defaults to None.
