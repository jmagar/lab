...
`is\_already\_in\_uv\_subprocess` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/utilities/cli.py#L28)
```
`is\_already\_in\_uv\_subprocess() -\> bool
`
```
...
`load\_and\_merge\_config` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/utilities/cli.py#L33)
```
`load\_and\_merge\_config(server\_spec: str | None, \*\*cli\_overrides) -\> tuple[MCPServerConfig, str]
`
```
...
**Args:**
* `server\_spec`: Python file, config file, URL, or None to auto-detect
* `cli\_overrides`: CLI arguments that override config values
**Returns:**
* Tuple of (MCPServerConfig, resolved\_server\_spec)
###
[​
](#log_server_banner)
`log\_server\_banner` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/utilities/cli.py#L201)
```
`log\_server\_banner(server: FastMCP[Any]) -\> None
`
```
