...
`check\_distributed\_backend` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/cli/tasks.py#L22)
```
`check\_distributed\_backend() -\> None
`
```
...
`worker` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/cli/tasks.py#L61)
```
`worker(server\_spec: Annotated[str | None, cyclopts.Parameter(help='Python file to run, optionally with :object suffix, or None to auto-detect fastmcp.json')] = None) -\> None
`
```
