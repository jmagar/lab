...
`EventEntry` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/server/event_store.py#L26)
Stored event entry.
###
[​
](#streameventlist)
`StreamEventList` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/server/event_store.py#L34)
List of event IDs for a stream.
###
[​
](#eventstore)
`EventStore` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/server/event_store.py#L40)
EventStore implementation backed by AsyncKeyValue.
...
* `max\_events\_per\_stream`: Maximum events to retain per stream. Default 100.
* `ttl`: Event TTL in seconds. Default 3600 (1 hour). Set to None for no expiration.
**Methods:**
####
[​
](#store_event)
`store\_event` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/server/event_store.py#L94)
```
`store\_event(self, stream\_id: StreamId, message: JSONRPCMessage | None) -\> EventId
`
```
Store an event and return its ID.
**Args:**
* `stream\_id`: ID of the stream the event belongs to
...
`replay\_events\_after` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/server/event_store.py#L135)
```
`replay\_events\_after(self, last\_event\_id: EventId, send\_callback: EventCallback) -\> StreamId | None
`
```
Replay events that occurred after the specified event ID.
**Args:**
* `last\_event\_id`: The ID of the last event the client received
* `send\_callback`: A callback function to send events to the client
**Returns:**
