...
`FormInput` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/apps/form.py#L57)
...
a form from it using `Form.from\_model()`. Field types, labels,
descriptions, and validation are all derived from the model.
Optionally provide an `on\_submit` callback to process the
