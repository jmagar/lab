...
`DereferenceRefsMiddleware` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/server/middleware/dereference.py#L15)
Dereferences $ref in component schemas before sending to clients.
Some MCP clients (e.g., VS Code Copilot) don’t handle JSON Schema refproperly.Thismiddlewareinlinesallref
properly. This middleware inlines all ref definitions so schemas are
self-contained. Enabled by default via `FastMCP(dereference\_schemas=True)`.
**Methods:**
####
[​
](#on_list_tools)
`on\_list\_tools` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/server/middleware/dereference.py#L24)
```
...
`on\_list\_resource\_templates` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/server/middleware/dereference.py#L33)
```
