...
`subscribe\_to\_task\_updates` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/server/tasks/subscriptions.py#L31)
```
`subscribe\_to\_task\_updates(task\_id: str, task\_key: str, session: ServerSession, docket: Docket, poll\_interval\_ms: int = 5000) -\> None
`
```
...
**Args:**
* `task\_id`: Client-visible task ID (server-generated UUID)
* `task\_key`: Internal Docket execution key (includes session, type, component)
* `session`: MCP ServerSession for sending notifications
* `docket`: Docket instance for subscribing to execution events
* `poll\_interval\_ms`: Poll interval in milliseconds to include in notifications