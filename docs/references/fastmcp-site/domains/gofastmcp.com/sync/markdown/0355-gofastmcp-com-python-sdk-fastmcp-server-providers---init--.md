...
self.db = Database(db\_url)
async def \_list\_tools(self) -\> list[Tool]:
rows = await self.db.fetch("SELECT \* FROM tools")
return [self.\_make\_tool(row) for row in rows]
async def \_get\_tool(self, name: str) -\> Tool | None:
row = await self.db.fetchone("SELECT \* FROM tools WHERE name = ?", name)
return self.\_make\_tool(row) if row else None
mcp = FastMCP("Server", providers=[DatabaseProvider(db\_url)])
`
```