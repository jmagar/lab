...
`fastmcp.utilities.json\_schema\_type`
...
Person = json\_schema\_to\_type(schema)
...
`json\_schema\_to\_type` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/utilities/json_schema_type.py#L111)
```
`json\_schema\_to\_type(schema: Mapping[str, Any], name: str | None = None) -\> type
`
```
...
NameType = json\_schema\_to\_type(schema)
# Creates Annotated[str, StringConstraints(min\_length=3, pattern="^[A-Z][a-z]+$")]
@dataclass
class Name:
name: NameType
`
...
`JSONSchema` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/utilities/json_schema_type.py#L78)