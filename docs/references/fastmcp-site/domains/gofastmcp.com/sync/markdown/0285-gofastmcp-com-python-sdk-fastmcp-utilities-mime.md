...
`resolve\_ui\_mime\_type` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/utilities/mime.py#L10)
```
`resolve\_ui\_mime\_type(uri: str, explicit\_mime\_type: str | None) -\> str | None
`
```
Return the appropriate MIME type for a resource URI.
For `ui://` scheme resources, defaults to `UI\_MIME\_TYPE` when no
explicit MIME type is provided.
**Args:**
* `uri`: The resource URI string
* `explicit\_mime\_type`: The MIME type explicitly provided by the user
**Returns:**
