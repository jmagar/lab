...
Registers tools and resources from `prefab\_ui.generative` so that an
...
`GenerativeUI` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/apps/generative.py#L53)
A Provider that adds generative UI capabilities to a server.
Registers:
* A `generate\_ui` tool that accepts Prefab Python code, executes
...
`lifespan` [](https://github.com/PrefectHQ/fastmcp/blob/main/src/fastmcp/apps/generative.py#L196)
```
