Writing Plugins | Gotify
[Skip to content](#VPContent)
Menu
Return to top
# Writing Plugins [​](#writing-plugins)
## Description [​](#description)
Lets take a closer look at the minimal plugin instance we created in the [intro](./plugin) part:
go
```
...
```
Now only the base [`Plugin`](https://godoc.org/github.com/gotify/plugin-api#Plugin) interface has been implemented. To give it more functionalities, more interfaces can be implemented.
## APIs [​](#apis)
### Intro [​](#intro)
...
*These are all API interfaces exposed by gotify. Some interface implementations are omitted in the examples.*
#### Displayer [​](#displayer)
[`Displayer`](https://godoc.org/github.com/gotify/plugin-api#Displayer) is the most simple form of plugin API, it is used to provide instructions on the plugin page in the WebUI. Plugins can dynamically generate information based on the current state. It receives a `location` parameter contains the server's hostname, port and scheme recovered from the original request to the `Displayer` API. Markdown is supported.
...
```
#### Messenger [​](#messenger)
[`Messenger`](https://godoc.org/github.com/gotify/plugin-api#Messenger) is used to send messages. It is called with a callback that plugin instances can call at any time to send messages to the user.
go
```
`// Plugin is the plugin instance
type Plugin struct {
...
```
#### Storager [​](#storager)
[`Storager`](https://godoc.org/github.com/gotify/plugin-api#Storager) is used to store permanent information on the gotify database on the user level. Data serialization is handled by the plugin itself.
go
```
`// Plugin is the plugin instance
type Plugin struct {
...
type Storage struct {
EnabledTimes uint `json:"enabled\_times"`
}
func (c \*Plugin) Enable() error {
storage := new(Storage)
...
```
#### Webhooker [​](#webhooker)
[`Webhooker`](https://godoc.org/github.com/gotify/plugin-api#Webhooker) is used to register custom gin handlers. The base path is the base path of the `RouterGroup`, which is kept consistent between restarts. Plugins can assemble an absolute webhook URL by combining `basePath` and `location` parameter in `Displayer` call. Useful to register webhook handlers. In theory you can even register a complete UI here.
go
```
`// Plugin is the plugin instance
type Plugin struct {
...
[`Configurer`](https://godoc.org/github.com/gotify/plugin-api#Configurer) is used to provide configuration interfaces to the user. Marshaling and Unmarshaling is handled by the gotify main program.
...
```
## Effective Plugin Practices [​](#effective-plugin-practices)
