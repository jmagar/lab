...
Requestors poll for task completion by sending [`tasks/get`](/specification/2025-11-25/schema#tasks/get) requests.
...
After a task completes the operation result is retrieved via [`tasks/result`](/specification/2025-11-25/schema#tasks/result). This is distinct from the initial `CreateTaskResult` response, which contains only task data. The result structure matches the original request type (e.g., `CallToolResult` for `tools/call`).
...
Task Status Notification
When a task status changes, receivers **MAY** send a [`notifications/tasks/status`](/specification/2025-11-25/schema#notifications/tasks/status) notification to inform the requestor of the change. This notification includes the full task state.
**Notification:**
```
`{
"jsonrpc": "2.0",
...
Listing Tasks
To retrieve a list of tasks, requestors can send a [`tasks/list`](/specification/2025-11-25/schema#tasks/list) request. This operation supports pagination.
**Request:**
```
`{
"jsonrpc": "2.0",
...
Cancelling Tasks
To explicitly cancel a task, requestors can send a [`tasks/cancel`](/specification/2025-11-25/schema#tasks/cancel) request.
**Request:**
```
`{
"jsonrpc": "2.0",
...
3. When the requestor encounters the `input\_required` status, it **SHOULD** preemptively call `tasks/result`.
...
3. When a receiver receives a `tasks/result` request for a task in any other non-terminal status (`working` or `input\_required`), it **MUST** block the response until the task reaches a terminal status.
