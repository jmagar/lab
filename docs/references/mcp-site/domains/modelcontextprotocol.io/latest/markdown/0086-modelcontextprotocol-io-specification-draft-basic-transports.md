...
4. The body of the HTTP POST request **MUST** be a single JSON-RPC *request*, *notification*, or *response* to a server-sent request.
5. If the body is a JSON-RPC *notification* or *response* to a server-sent request:
...
it **SHOULD** send an SSE event with a standard [`retry`](<https://html.spec.whatwg.org/multipage/server-sent-events.html#:~:text=field name is "retry">) field before
...
*request* from the client, **except** that `roots/list`,
