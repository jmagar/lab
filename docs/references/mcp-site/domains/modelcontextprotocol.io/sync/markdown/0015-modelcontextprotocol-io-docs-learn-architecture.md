...
```
`# Pseudo Code
async with stdio\_client(server\_config) as (read, write):
async with ClientSession(read, write) as session:
init\_response = await session.initialize()
if init\_response.capabilities.tools:
app.register\_mcp\_server(session, supports\_tools=True)
app.set\_server\_ready(session)
`
```
2
[
...
available\_tools = []
for session in app.mcp\_server\_sessions():
tools\_response = await session.list\_tools()
available\_tools.extend(tools\_response.tools)
conversation.register\_available\_tools(available\_tools)
`
```
3
[
...
async def handle\_tool\_call(conversation, tool\_name, arguments):
session = app.find\_mcp\_session\_for\_tool(tool\_name)
result = await session.call\_tool(tool\_name, arguments)
conversation.add\_tool\_result(result.content)
`
```
4
[
...
async def handle\_tools\_changed\_notification(session):
tools\_response = await session.list\_tools()
app.update\_available\_tools(session, tools\_response.tools)
if app.conversation.is\_active():
app.conversation.notify\_llm\_of\_new\_capabilities()
`
```