Install as a daemon · Tautulli/Tautulli Wiki · GitHub
...
JonnyWong16 edited this page Mar 9, 2021
&middot;
[33 revisions](/Tautulli/Tautulli/wiki/Install-as-a-daemon/_history)
...
## Linux
[](#linux)
Refer to the main [Installation](/Tautulli/Tautulli/wiki/Installation) instructions.
Deprecated instructions
...
* Enable at boot:
```
`sudo sysrc tautulli\_enable="YES"
`
```
* To start:
```
...
* **Note:** You may ignore the warning `/usr/local/etc/rc.d/tautulli: WARNING: $command\_interpreter /usr/local/bin/python3 != python`.
Optional:
* If you need to change user:
```
`sudo sysrc tautulli\_user="USERNAME"
`
```
...
```
`sudo sysrc tautulli\_dir="DIRECTORY"
`
```
## FreeNAS
[](#freenas)
...
* Set user to run:
```
`sysrc tautulli\_user="root"
`
```
* Enable at boot:
```
`sysrc tautulli\_enable="YES"
`
```
* To start:
```
