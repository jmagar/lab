...
JonnyWong16 edited this page Aug 17, 2024
&middot;
[130 revisions](/Tautulli/Tautulli/wiki/Frequently-Asked-Questions/_history)
**If you can't find a solution here, please ask on [Discord](https://discord.gg/tQcWEUp). Don't post questions on the GitHub issues tracker.**
## General
[](#general)
...
* [I get a warning that my Docker volume mount is not configured properly.](#docker-mount)
* [Where is my Tautulli data (`config.ini`/`tautulli.db`) saved?](#data-location)
...
## Snap
[](#snap)
* [How do I allow Tautulli to access `/media` and `/mnt` locations?](#snap-mounts)
### General
[](#general-1)
####
...
[](#q-i-forgot-my-username-andor-password)
...
3. Search for `http\_username` and `http\_password` and the values after the equal signs are the username and password. You can also delete both lines from the file to disable authentication.
...
[](#q-i-cant-reach-the-web-interface)
**A:** Shut down Tautulli and open the Tautulli `config.ini` file with your preferred text editor, search for the lines that begin with `http\_host`, `http\_port`, and `http\_root` and remove the entire line. You should also remove the line for `enable\_https` from the file. After you have removed these lines from `config.ini`, go ahead and start Tautulli. It will listen on the default IP address and port (`http://localhost:8181`).
####
...
```
`http://localhost:8181/delete\_duplicate\_libraries
`
```
...
proxy\_set\_header X-Forwarded-Ssl $xforwardedssl;
proxy\_redirect \~^(http(?:s)?://)([^:/]+)(?::\\d+)?(/.\*)?$ $1$2:$server\_port$3;
}
```
Don't forget to clear your web browser's cache *every* time you update your web server configuration.
####
Q: I need to move/reinstall Tautulli. Can I keep my history and statistics?
[](#q-i-need-to-movereinstall-tautulli-can-i-keep-my-history-and-statistics)
...
[](#q-help-i-accidentally-deleted-a-userlibrary-how-can-i-add-it-back)
...
##### Undeleting a user
[](#undeleting-a-user)
If you are trying to undelete a user first you will need to find their `user\_id`. You can get this by following the "More XML Shortcuts" instructions [here](Debugging#more-xml-shortcuts) for "All Users".
You will see a list of items, find the entry that corresponds to the user you want to undelete, the `user\_id` is in the `id="12345"` part. So for example in this entry the `user\_id` would be `8367478`:
```
\<User id="8367478" title="exampleuser" username="exampleuser" email="example@domain.com" ...\>
\<Server id="1234567" name="Server Name" ... /\>
\</User\>
```
Once you have the ID you need to go to `/undelete\_user?user\_id=\<user\_id\>` on your Tautulli instance. For example for the above ID on a Tautulli instance on the same machine you would go to:
```
`http://localhost:8181/undelete\_user?user\_id=8367478
`
```
If you are unable to find the `user\_id`, you may try using the username (`/undelete\_user?username=\<username\>`) or their email address (`/undelete\_user?username=\<user's Plex email address\>`), but these are less accurate and may not work. Example usage of these for a Tautulli instance running on the same computer would be:
```
`http://localhost:8181/undelete\_user?username=exampleuser
http://localhost:8181/undelete\_user?username=example@domain.com
`
```
##### Undeleting a library
[](#undeleting-a-library)
If you are trying to undelete a library you will need to find the `section\_id`. You can get this by following the "More XML Shortcuts" instructions [here](Debugging#more-xml-shortcuts) for "All Libraries".
You will see a list of items, find the entry that corresponds to the library you want to undelete, the `section\_id` is in the `key="2"` part. So for example in this entry the `section\_id` would be `3`:
```
\<Directory ... key="3" type="show" title="TV Shows" ...\>
\<Location id="7" path="/mnt/TV Shows"/\>
\</Directory\>
```
Once you have the section ID you need to go to `/undelete\_library?section\_id=\<section\_id\>` on your Tautulli instance. For example for the above ID on a Tautulli instance on the same machine you would go to:
```
`http://localhost:8181/undelete\_library?section\_id=3
`
```
If you are unable to find the `section\_id`, you may try using the library name (`/undelete\_library?section\_name=\<section\_name\>`), but this is less accurate and may not work. Example usage for a Tautulli instance running on the same computer would be:
```
`http://localhost:8181/undelete\_library?section\_name=Anime
`
```
####
Q: Can I import my history from PlexWatch/Plexivity?
[](#q-can-i-import-my-history-from-plexwatchplexivity)
...
[](#q-where-can-i-find-my-tautulli-logs)
**A:** You can view the Tautulli logs from the web interface by clicking on "View Logs" in the settings menu. Clicking on the "Download logs" button on this page will allow you to save a copy of the log file. The location of the log file is also listed on the Settings \> Help & Info page. If Tautulli is unable to start, then the default log file location can be found [here](/Tautulli/Tautulli/wiki/Asking-for-Support#how-can-i-share-my-logs). When reporting an issue, please provide a link to this log file by pasting it on [Gist](http://gist.github.com), *do not* upload the file as an attachment.
####
...
[](#q-tautulli-cant-connect-to-my-plex-server-locally-with-use-ssl-checked)
...
####
Q: My Tautulli database got corrupted: "DatabaseError: database disk image is malformed" or "sqlite3.OperationalError".
[](#q-my-tautulli-database-got-corrupted-databaseerror-database-disk-image-is-malformed-or-sqlite3operationalerror)
...
3. Go to "Tools \> Integrity Check", then click "OK" to run `PRAGMA pragma integrity\_check;` ([screenshot](./images/pragma_integrity_check.png)). If it does not say "ok" then you need to repair your database.
...
* *(Optional)* Go to the "Execute SQL" tab and run `pragma integrity\_check;` and it should say "ok".
* Go to "File \> Close Database".
* Replace your Tautulli database (`tautulli.db`) file with this new one.
*Note:*
If you run into issues during step 9 above when building a new database from your exported SQL file the first thing to try would be a minimal export that *only* saves your history and settings. To do this:
...
* Continue running through the steps above. If you run into issues again with the minimal table list it's still possible to recover data, but the process is very involved so we ask that you contact [support in Discord](https://tautulli.com/discord) in order to proceed.
####
...
####
Q: Tautulli won't start due to a corrupted config file
[](#q-tautulli-wont-start-due-to-a-corrupted-config-file)
...
`
```
If you would like to opt-out of data collection, you can set `system\_analytics = 0` in the `config.ini` file. Tautulli must be shut down when editing the file.
####
Q: I'm seeing an "Unable to connect to remote host because of a SSL error." message when trying to update.
[](#q-im-seeing-an-unable-to-connect-to-remote-host-because-of-a-ssl-error-message-when-trying-to-update)
...
If after performing all of the above steps have not solved your issue please join the `#support` channel of our [Discord server](https://discord.gg/tQcWEUp) as there can be *many* platform specific issues related to this.
####
...
####
Q: My media info table is not loading.
[](#q-my-media-info-table-is-not-loading)
**A:** Clear the media info table cache by going to the following URL, where `XX` is the ID number for the library.
```
`http://localhost:8181/delete\_media\_info\_cache?section\_id=XX
`
```
####
Q: I get a warning that my Docker volume mount is not configured properly.
[](#q-i-get-a-warning-that-my-docker-volume-mount-is-not-configured-properly)
**A:** The Docker volume mount for the `/config` folder is used to save all of your Tautulli data (history, settings, etc.). Make sure you have added a *bind* volume mount from your host system to the `/config` folder of your Docker container. Refer to the [Installation](/Tautulli/Tautulli/wiki/Installation) instructions for more details. A bind volume mount will save all the data to your host system instead of inside the container itself. If you do not provide a volume mount, your data may be cleared when the container is recreated or updated.
*Note:* Tautulli is unable to detect named Docker volume mounts so the warning message may still appear. If you are using a named Docker volume instead of a bind volume mount then you may ignore the message or disable the check completely by setting `check\_docker\_mount = 0` in your `config.ini` file.
####
Q: Where is my Tautulli data (`config.ini`/`tautulli.db`) saved?
[](#q-where-is-my-tautulli-data-configinitautullidb-saved)
**A:** The Tautulli data folder stores all your history, settings, backups, logs, etc. This includes the `tautulli.db` database file and your settings `config.ini` file. The easiest way to find the location of these files is to check the Settings \> Help & Info page.
If you are unable to check the Tautulli settings page, the follow are the default locations for the Tautulli data:
* Windows exe installation: `%LOCALAPPDATA%\\Tautulli`
* macOS pkg installation: `\~/Library/Application Support/Tautulli`
* Snap package installation: `/root/snap/tautulli/common`
* Docker installation: The `\<path to data\>` folder from your bind volume mount
* Git or zip based installation: The installation folder where you cloned the git repository or extracted the zip file
### Activity and History Logging
[](#activity-and-history-logging-1)
####
...
*Note:* If you are experiencing errors in the log, such as the [`DatabaseError: database disk image is malformed`](#general-q18) error, you should fix those first *before* attempting to flush sessions above.
####
...
[](#q-watch-history-isnt-showing-up-under-recently-watched-on-the-homepage)
...
```
`http://localhost:8181/info?rating\_key=YY
`
```
...
```
` DELETE FROM session\_history WHERE id NOT IN (SELECT id FROM session\_history\_metadata);
DELETE FROM session\_history\_media\_info WHERE id NOT IN (SELECT id FROM session\_history\_metadata);
`
```
...
[](#q-im-seeing-the-same-artist-twice-on-the-homepage)
...
####
Q: Why are items showing up as 127.0.0.1 for the IP address?
[](#q-why-are-items-showing-up-as-127001-for-the-ip-address)
...
####
Q: My playback stop notifications are not working.
[](#q-my-playback-stop-notifications-are-not-working)
...
[](#q-can-i-disable-history-logging-for-a-specific-userlibrary)
...
####
Q: I played multiple shows on Live TV but Tautulli says I only watched one show the entire time.
[](#q-i-played-multiple-shows-on-live-tv-but-tautulli-says-i-only-watched-one-show-the-entire-time)
...
####
Q: How do I disable history logging for Live TV?
[](#q-how-do-i-disable-history-logging-for-live-tv)
...
####
Q: Can I view the history for an entire TV show I watched on Live TV similar to TV shows in my own library?
[](#q-can-i-view-the-history-for-an-entire-tv-show-i-watched-on-live-tv-similar-to-tv-shows-in-my-own-library)
**A:** No, this is currently not possible. You can only view history for a single episode at a time on the info pages.
####
Q: How do I hide Live TV on the Graphs page?
[](#q-how-do-i-hide-live-tv-on-the-graphs-page)
...
[](#q-i-cant-get-notifications-working-is-there-a-magic-trick)
...
[](#q-my-tests-say-they-are-successful-but-the-notification-isnt-sent-and-theres-nothing-in-the-logs)
...
[](#q-can-i-disable-notifications-for-a-specific-userlibrary)
**A:** You can control which users/libraries will send a notification by setting up a [custom condition](/Tautulli/Tautulli/wiki/Custom-Notification-Conditions) in your notification agent settings.
####
Q: Can I disable recently added notifications for TV Shows/Movies?
[](#q-can-i-disable-recently-added-notifications-for-tv-showsmovies)
...
[](#q-im-seeing-a-the-pycryptodome-library-is-missing-message)
...
[](#q-notifications-are-sending-the-wrong-textsending-the-default-text)
...
####
Q: My recently added notifications are not sending and the logs say "Not notifying again".
[](#q-my-recently-added-notifications-are-not-sending-and-the-logs-say-not-notifying-again)
...
[](#q-i-enabled-a-library-but-its-not-showing-in-the-newsletter)
...
It's recommended to copy the current template to start off with. Download [recently\_added.html](https://github.com/Tautulli/Tautulli/raw/master/data/interfaces/newsletters/recently_added.html) to a folder of your choice. You can then edit the `recently\_added.html` file to match what you would like the newsletters to look like. The templating language in use here is [Mako](http://www.makotemplates.org/).
*Note: Your folder can be located anywhere Tautulli has access to, but the file **must** be named `recently\_added.html`.*
After you have that set up, you need to tell Tautulli to use the new template. Do this by filling in the **full** path to the folder you created above in the Settings \> Notifications & Newsletters \> Show Advanced button \> [Custom Newsletter Template Folder](./images/newsletter_custom_template_folder.png) option.
...
```
`http://localhost:8181/newsletter\_preview?newsletter\_id=1&raw=true
`
```
### Windows
[](#windows-1)
...
[](#q-tautulli-keeps-telling-me-youre-running-an-unknown-version-of-tautulli)
...
>
> Run the installer, select all the defaults except for the section called "Adjusting your PATH environment" - here select
**> "Git from the command line and also from 3rd-party software"
**> .
>
*Note: If you are seeing this and aren't running Windows, it's likely you are hitting the [SSL error](#ssl-update) FAQ entry!*
####
...
[](#q-when-trying-to-update-the-logs-say-agreeing-to-the-xcodeios-license-requires-admin-privileges-please-re-run-as-root-via-sudo)
...
[](#q-im-getting-a-valueerror-unknown-locale-utf-8-message)
**A:** If you are seeing this error when trying to start Tautulli the simple fix is to add these lines to your `\~/.bash\_profile` file:
```
...
```
####
Q: How do I get rid of the Python rocket icon in my dock?
[](#q-how-do-i-get-rid-of-the-python-rocket-icon-in-my-dock)
**A:** Unfortunately, the Python dock icon is required when the menu bar icon is enabled and you are running Tautulli using Python directly. You can either disable the menu bar icon, or reinstall Tautulli using the macOS pkg instead (refer to this [wiki page](/Tautulli/Tautulli/wiki/Upgrading-to-Python-3-(Tautulli-v2.5)#windows--macos) for details). The pkg install will not show any dock icons regardless if the menu bar icon is enabled or disabled.
### QNAP
[](#qnap-1)
####
...
```
`mkdir /share/MD0\_DATA/PATH/TO/VALID/SHARE/FOLDER/PlexLogs
`
```
3. Create symbolic link between unshared log folder and new shared folder:
```
`ln -s "/share/MD0\_DATA/.qpkg/PlexMediaServer/Library/Plex Media Server/Logs/" "/share/MD0\_DATA/PATH/TO/VALID/SHARE/FOLDER/PlexLogs"
`
```
...
[](#snap-1)
####
Q: How do I allow Tautulli to access `/media` and `/mnt` locations?
[](#q-how-do-i-allow-tautulli-to-access-media-and-mnt-locations)
**A:** Tautulli cannot access locations under `/media` and `/mnt` by default and must be enabled manually. Run the following command to enable access:
```
`sudo snap connect tautulli:removable-media :removable-media
`
```
Note: The root user needs write access if you intend to write to a mount inside `/media` or `/mnt`.
