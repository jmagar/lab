...
On this page
Having trouble? Find solutions to common issues below.
**Start here: run **`**openacp doctor**`
The built-in diagnostic command checks your configuration, bot tokens, agent commands, and platform connectivity automatically. It catches the majority of setup issues in seconds:
Copy
```
`openacp doctor`
```
Each check reports a pass, warning, or failure with a specific message — fix the reported issues first before diving into the guides below.
##
[](#guides-by-topic)
Guides by topic
*
[Telegram issues](/troubleshooting/telegram-issues) — bot not responding, missing rights, topics, rate limits
*
[Discord issues](/troubleshooting/discord-issues) — slash commands, intents, thread creation, interactions
*
[Slack issues](/troubleshooting/slack-issues) — Socket Mode, token types, interactivity, channel creation
*
[Agent issues](/troubleshooting/agent-issues) — command not found, crashes, timeouts, dependency errors
*
[FAQ](/troubleshooting/faq) — supported platforms, multiple users, costs, privacy, offline support
[PreviousEnvironment Variables](/api-reference/environment-variables)[NextTelegram Issues](/troubleshooting/telegram-issues)
