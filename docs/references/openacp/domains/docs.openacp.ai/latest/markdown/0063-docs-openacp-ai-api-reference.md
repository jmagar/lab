...
[Configuration Schema](/api-reference/configuration-schema)
Full `\<instance-root\>/config.json` field reference
[Environment Variables](/api-reference/environment-variables)
`OPENACP\_\*` env vars and their config equivalents
...
[PreviousContributing](/extending/contributing)[NextCLI Commands](/api-reference/cli-commands)
