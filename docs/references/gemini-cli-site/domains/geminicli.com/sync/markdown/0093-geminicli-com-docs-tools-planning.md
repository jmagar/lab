Gemini CLI planning tools | Gemini CLI
...
plan to the user.
## 1. `enter\_plan\_mode` (EnterPlanMode)
[Section titled “1. enter\_plan\_mode (EnterPlanMode)”](#1-enter_plan_mode-enterplanmode)
`enter\_plan\_mode` switches the CLI to Plan Mode. This tool is typically called
...
## 2. `exit\_plan\_mode` (ExitPlanMode)
[Section titled “2. exit\_plan\_mode (ExitPlanMode)”](#2-exit_plan_mode-exitplanmode)
`exit\_plan\_mode` signals that the planning phase is complete. It presents the
...
* **Tool name:** `exit\_plan\_mode`
* **Display name:** Exit Plan Mode
* **File:** `exit-plan-mode.ts`
* **Parameters:**
* `plan\_path` (string, required): The path to the finalized Markdown plan
file. This file MUST be located within the project’s temporary plans
directory (for example, `\~/.gemini/tmp/\<project\>/plans/`).
* **Behavior:**
* Validates that the `plan\_path` is within the allowed directory and that the
