File management with Gemini CLI | Gemini CLI
...
[Section titled “Providing context by reading files”](#providing-context-by-reading-files)
...
[Section titled “Direct file inclusion (@)”](#direct-file-inclusion)
...
[Section titled “Working with multiple files”](#working-with-multiple-files)
...
[Section titled “Including entire directories”](#including-entire-directories)
...
[Section titled “How to find files (Exploration)”](#how-to-find-files-exploration)
If you *don’t* know the exact file path, you can ask Gemini CLI to find it for
you. This is useful when navigating a new codebase or looking for specific
logic.
### Scenario: Find a component definition
[Section titled “Scenario: Find a component definition”](#scenario-find-a-component-definition)
...
`
```
Gemini uses the `glob` or `list\_directory` tools to search your project
...
## How to modify code
[Section titled “How to modify code”](#how-to-modify-code)
...
### Creating new files
[Section titled “Creating new files”](#creating-new-files)
...
`
```
Gemini CLI uses the `write\_file` tool to generate the new file from scratch.
## Review and confirm changes
[Section titled “Review and confirm changes”](#review-and-confirm-changes)
...
## Verify the result
[Section titled “Verify the result”](#verify-the-result)
...
`
```
Gemini CLI uses the `run\_shell\_command` tool to execute your test runner (for
example, `npm test` or `jest`). This ensures the changes didn’t break existing
functionality.
## Advanced: Controlling what Gemini sees
[Section titled “Advanced: Controlling what Gemini sees”](#advanced-controlling-what-gemini-sees)
By default, Gemini CLI respects your `.gitignore` file. It won’t read or search
through `node\_modules`, build artifacts, or other ignored paths.
