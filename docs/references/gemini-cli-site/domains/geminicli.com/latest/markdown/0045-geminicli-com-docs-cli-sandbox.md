Sandboxing in Gemini CLI | Gemini CLI
...
[Section titled “Using the command flag”](#using-the-command-flag)
Terminal window
```
`
...
```
`
$env:GEMINI\_SANDBOX="true"
gemini -p "run the test suite"
`
```
### Configuring via settings.json
[Section titled “Configuring via settings.json”](#configuring-via-settingsjson)
```
`
{
"tools": {
...
`GEMINI\_SANDBOX=true|docker|podman|sandbox-exec|runsc|lxc`
...
Built-in profiles (set via `SEATBELT\_PROFILE` env var):
...
set the `GEMINI\_SANDBOX\_IMAGE` environment variable.
...
```
`
export GEMINI\_SANDBOX\_IMAGE="us-central1-docker.pkg.dev/my-project/my-repo/my-custom-sandbox:latest"
`
```
...
3. Run your command with the `BUILD\_SANDBOX` environment variable set:
Terminal window
```
`
BUILD\_SANDBOX=1 GEMINI\_SANDBOX=docker gemini -p "run my custom build"
`
```
### 3. Windows Native Sandbox (Windows only)
[Section titled “3. Windows Native Sandbox (Windows only)”](#3-windows-native-sandbox-windows-only)
...
[Section titled “4. gVisor / runsc (Linux only)”](#4-gvisor--runsc-linux-only)
...
`GEMINI\_SANDBOX=runsc` or `sandbox: "runsc"`).
To set up runsc:
1. Install the runsc binary.
2. Configure the Docker daemon to use the runsc runtime.
3. Verify the installation.
### 5. LXC/LXD (Linux only, experimental)
[Section titled “5. LXC/LXD (Linux only, experimental)”](#5-lxclxd-linux-only-experimental)
...
`
export GEMINI\_SANDBOX=lxc
export GEMINI\_SANDBOX\_IMAGE=my-snapcraft-container
...
## Tool sandboxing
[Section titled “Tool sandboxing”](#tool-sandboxing)
Tool-level sandboxing provides granular isolation for individual tool executions
(like `shell\_exec` and `write\_file`) instead of sandboxing the entire Gemini CLI
process.
...
[Section titled “How to turn off tool sandboxing”](#how-to-turn-off-tool-sandboxing)
...
CLI to take effect.
## Sandbox expansion
[Section titled “Sandbox expansion”](#sandbox-expansion)
...
[Section titled “How sandbox expansion works”](#how-sandbox-expansion-works)
...
[Section titled “Including files outside the workspace”](#including-files-outside-the-workspace)
...
using the `SANDBOX\_MOUNTS` environment variable.
...
```
`
export SANDBOX\_MOUNTS="/path/on/host:/path/in/container:rw,/another/path:ro"
`
```
## Running inside a Docker container
[Section titled “Running inside a Docker container”](#running-inside-a-docker-container)
...
`podman` command using the `SANDBOX\_FLAGS` environment variable. This is useful
...
```
`
export SANDBOX\_FLAGS="--security-opt label=disable"
`
```
**Windows (PowerShell)**
Terminal window
```
`
$env:SANDBOX\_FLAGS="--security-opt label=disable"
`
```
...
```
`
export SANDBOX\_FLAGS="--flag1 --flag2=value"
`
```
**Windows (PowerShell)**
Terminal window
```
`
$env:SANDBOX\_FLAGS="--flag1 --flag2=value"
`
```
...
```
`
export SANDBOX\_SET\_UID\_GID=true # Force host UID/GID
export SANDBOX\_SET\_UID\_GID=false # Disable UID/GID mapping
`
```
**Windows (PowerShell)**
Terminal window
```
`
$env:SANDBOX\_SET\_UID\_GID="true" # Force host UID/GID
$env:SANDBOX\_SET\_UID\_GID="false" # Disable UID/GID mapping
`
```
## Troubleshooting
...
**Missing commands**
* Add to a custom Dockerfile. Automatic `BUILD\_SANDBOX` builds are only
