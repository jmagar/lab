...
* **API key management:** It handles the `GEMINI\_API\_KEY` and ensures it’s used
securely when communicating with the Gemini API.
* **Tool execution:** When tools interact with the local system (for example,
`run\_shell\_command`), the core (and its underlying tool implementations) must
