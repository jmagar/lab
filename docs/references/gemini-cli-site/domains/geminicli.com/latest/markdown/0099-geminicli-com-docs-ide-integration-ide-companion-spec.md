Gemini CLI companion plugin: Interface specification | Gemini CLI
...
set the `GEMINI\_CLI\_IDE\_SERVER\_PORT` environment variable in the integrated
...
`GEMINI\_CLI\_IDE\_SERVER\_PORT` variable to identify and connect to the correct
window’s server.
...
3. **On deactivation (IDE shutdown/plugin disabled):**
