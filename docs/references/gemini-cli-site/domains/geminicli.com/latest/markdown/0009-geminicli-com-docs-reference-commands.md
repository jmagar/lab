...
checkpoints are:
* Linux/macOS: `\~/.gemini/tmp/\<project\_hash\>/`
* Windows: `C:\\Users\\\<YourUsername\>\\.gemini\\tmp\\\<project\_hash\>\\`
...
### `/shells` (or `/bashes`)
[Section titled “/shells (or /bashes)”](#shells-or-bashes)
...
### `/setup-github`
[Section titled “/setup-github”](#setup-github)
...
### `/terminal-setup`
[Section titled “/terminal-setup”](#terminal-setup)
...
* `@src/my\_project/ Summarize the code in this directory.`
...
* The command uses the `read\_many\_files` tool internally. The content is
...
`node\_modules/`, `dist/`, `.env`, `.git/`) are excluded. This behavior can
...
skipped or truncated by the underlying `read\_many\_files` tool to ensure
...
`read\_many\_files` was used, along with a message detailing the status and
...
* If the `read\_many\_files` tool encounters an error (for example, permission
issues), this will also be reported.
## Shell mode and passthrough commands (`!`)
[Section titled “Shell mode and passthrough commands (!)”](#shell-mode-and-passthrough-commands)
The `!` prefix lets you interact with your system’s shell directly from within
Gemini CLI.
* **`!\<shell\_command\>`**
* **Description:** Execute the given `\<shell\_command\>` using `bash` on
...
the `GEMINI\_CLI=1` environment variable is set in the subprocess’s
