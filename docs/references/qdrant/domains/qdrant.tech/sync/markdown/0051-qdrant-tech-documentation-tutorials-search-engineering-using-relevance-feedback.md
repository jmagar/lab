...
In this tutorial, you&rsquo;ll see how to:
...
3. Evaluate the gains it brings to this pipeline.## Relevance Feedback
...
### Strategy
To use the feedback for guiding a retriever in the vector space, there are several possible strategies. For now, only the **naive strategy** is available &ndash; [a simple 3-parameter formula](https://qdrant.tech/documentation/search/search-relevance/#naive-strategy) which adjusts similarity scoring based on the feedback.
...
`
```
We&rsquo;re going to use a [Qdrant Cloud Free Tier Cluster](https://qdrant.tech/documentation/cloud/create-cluster/#free-clusters) (make sure it&rsquo;s on version 1.17.0+) and [Free Embedding Inference](https://qdrant.tech/documentation/cloud/inference/#free-embedding-models) available on this Free Tier Cluster.
...
cloud\_inference=True # to be able to use free embedding inference on Qdrant Cloud, for convenience & to reduce latency
)
`
```
#### Dataset
We&rsquo;ll use one of the datasets available in the Qdrant Cluster UI.
1. Go to Cluster UI -\> Datasets.
2. Out of 3 datasets there, pick the first one, &ldquo;Qdrant Web Documentation&rdquo;. It consists of small text chunks from [our documentation website](https://qdrant.tech/documentation/).
3. Press the `Import` button and type in a collection name &ndash; `documentation`.
After a couple of seconds you should see a green status pop up *&ldquo;Snapshot successfully imported&rdquo;* and a collection named `documentation` in the list of all collections.
Now we have a collection to work with and optimize semantic search in.
```
`COLLECTION\_NAME = "documentation"
`
```
#### Retriever
The **retriever** is an embedding model that converts your raw data into vectors for semantic similarity search. The Relevance Feedback Query will help you optimize your retriever&rsquo;s ability to find relevant results in your data.
Our retriever is already defined by our collection &ndash; as the `all-MiniLM-L6-v2` tag on the dataset&rsquo;s &ldquo;VECTORS CONFIG&rdquo; shows us, vectors in the collection are produced with the `all-MiniLM-L6-v2` model.
Let&rsquo;s define it in our `qdrant-relevance-feedback` framework.
```
`from qdrant\_relevance\_feedback.retriever import QdrantRetriever
...
`> all-MiniLM-L6-v2
`> is covered by Qdrant Free Tier Cloud Inference. You can define and provide your custom retrievers, too.
Check what a point in this collection looks like.
Our documentation collection has only one vector per point &ndash; `Default vector`. However, in Qdrant, one can have several [named vectors](https://qdrant.tech/documentation/manage-data/vectors/#named-vectors) per point.
We need to provide the name of the vector associated with the retriever that we&rsquo;re planning to optimize with feedback.
```
`RETRIEVER\_VECTOR\_NAME = None # None if it's a default vector or your named vector handle in Qdrant's collection
`
```
We also need to point our framework to the raw data which is vectorized with our retriever model. Here, the raw data is the `text` field in the point&rsquo;s [payload](https://qdrant.tech/documentation/manage-data/payload/#payload). Simply put, we search on `text` snippets &ndash; in their vectorized form.
```
`PAYLOAD\_KEY = "text"
`
```
*If your raw data is stored elsewhere outside of Qdrant, you can redefine the `payload\_retrieval` part in `qdrant-relevance-feedback`.*
#### Feedback Model
...
As a provider of a feedback model in this tutorial, we&rsquo;ll use Qdrant&rsquo;s lightweight inference library called [FastEmbed](https://qdrant.tech/documentation/fastembed/).
```
`pip install fastembed
`
```
As `all-MiniLM-L6-v2` is a small (only 384 dimensions) and weak model, we don&rsquo;t have to pick a feedback model which is too strong or expensive. Such a small retriever won&rsquo;t be receptive to sophisticated feedback anyway.
...
```
`from qdrant\_relevance\_feedback.feedback import FastembedFeedback
...
```
`from qdrant\_relevance\_feedback import RelevanceFeedback
relevance\_feedback = RelevanceFeedback(
retriever=retriever,
feedback=feedback,
client=client,
collection\_name=COLLECTION\_NAME,
vector\_name=RETRIEVER\_VECTOR\_NAME,
payload\_key=PAYLOAD\_KEY
)
`
```
...
We&rsquo;ve generated 50 queries with Claude Code, using the prompt *&ldquo;Generate 50 short search queries (3-10 words each) that a developer might type when browsing Qdrant&rsquo;s website.&rdquo;*.
Training queries
```
`train\_queries = [
...
"qdrant ef\_construct m parameter tuning",
...
`
```
The bigger the `TRAIN\_LIMIT`, the more training data our formula gets, but the more expensive and slow the training becomes.
...
```
`formula\_params = relevance\_feedback.train(
queries=train\_queries,
limit=TRAIN\_LIMIT,
)
`
```
You&rsquo;ll see a &ldquo;**Building training data**&rdquo; process running on 50 queries.
...
Then after a blazingly fast training, you&rsquo;ll get your parameters, something like:
```
...
Let&rsquo;s see how introducing the Relevance Feedback Query to a retrieval pipeline could work on this use case.
**Reason:** Let&rsquo;s say you&rsquo;re powering documentation search with `all-MiniLM-L6-v2`, as it&rsquo;s very cheap and fast, but often it doesn&rsquo;t provide relevant enough results and you know there are more relevant parts of documentation than what `all-MiniLM-L6-v2` gets you.
For example, let&rsquo;s look at this query:
```
`query = "recommendations API how to use"
`
```
### 1. Vanilla Initial Retrieval
We run simple semantic similarity search with our retriever, as we&rsquo;d normally do in our search application.
```
`query\_embedding = retriever.embed\_query(query)
CONTEXT\_LIMIT = 3
responses = client.query\_points(
collection\_name=COLLECTION\_NAME,
query=query\_embedding,
with\_payload=True,
with\_vectors=True,
limit=CONTEXT\_LIMIT,
using=RETRIEVER\_VECTOR\_NAME,
).points
`
```
Let&rsquo;s see what results we get:
```
`responses\_raw = [r.payload[PAYLOAD\_KEY] for r in responses]
print("\\n--- Initial Retrieval Results ---")
for i, text in enumerate(responses\_raw, 1):
print(f" [{i}] {text[:200]}")
print()
`
```
We&rsquo;ll get something like:
```
`--- Initial Retrieval Results ---
...
`
```
*We&rsquo;re seeing duplicates as our collection consists of chunks from our documentation website and some text is identical across different sections.*
Works, but perhaps there&rsquo;s something else in our collection that would answer the query better &ndash; the retriever is just too weak to pick it up.
### 2. Getting Feedback on Initial Retrieval
Now we get feedback from our `mxbai-embed-large-v1` feedback model on the top 3 results for the query *&ldquo;recommendations API how to use&rdquo;*.
The feedback model rescores them according to its own judgment of semantic similarity. We only show it a small number of results (`CONTEXT\_LIMIT` = 3) to keep the pipeline fast and cheap.
```
`feedback\_model\_scores = feedback.score(query, responses\_raw)
`
```
...
responses\_point\_ids = [p.id for p in responses] # CONTEXT\_LIMIT responses from initial vanilla retrieval (their point IDs)
#responses\_vectors = [p.vector for p in responses]
relevance\_feedback\_responses = client.query\_points(
collection\_name=COLLECTION\_NAME,
query=models.RelevanceFeedbackQuery(
relevance\_feedback=models.RelevanceFeedbackInput(
target=query\_embedding,
feedback=[
models.FeedbackItem(example=example, score=score)
for example, score in zip(responses\_point\_ids, feedback\_model\_scores) # or zip(responses\_vectors, feedback\_model\_scores)
],
strategy=models.NaiveFeedbackStrategy(
naive=models.NaiveFeedbackStrategyParams(\*\*formula\_params) # our parameters from above
),
)
),
with\_payload=True,
...
`
```
Let&rsquo;s see what relevance feedback-based retrieval brings to the table:
```
`relevance\_feedback\_responses\_raw = [r.payload[PAYLOAD\_KEY] for r in relevance\_feedback\_responses]
print("\\n--- Additional Results from Relevance Feedback-based Retrieval ---")
for i, text in enumerate(relevance\_feedback\_responses\_raw, 1):
print(f" [{i}] {text[:200]}")
print()
`
```
...
*Approach is tied to what you&rsquo;re passing to the `example` field in `FeedbackItem` (or analogues in other clients): raw retriever-produced embeddings or point IDs from your collection.*
> If you&rsquo;re passing point IDs as
`> example
`> s, these points are automatically excluded from the results. To include them among other points, pass the raw vectors (see commented
`> responses_vectors
`> ) instead of point IDs.
## Evaluation
...
Analogously, we&rsquo;ve generated a test set of queries.
Test queries
```
`test\_queries = [
...
`
```
Now let&rsquo;s run our evaluation on this test set, and see how much gain we get, considering that our feedback model receives `CONTEXT\_LIMIT` results per query to provide feedback on.
```
`from qdrant\_relevance\_feedback.evaluate import Evaluator
N = 10 # as in metric@N
evaluator = Evaluator(relevance\_feedback=relevance\_feedback)
results = evaluator.evaluate\_queries(
at\_n=N,
formula\_params=formula\_params, # from above
eval\_queries=test\_queries,
eval\_context\_limit=CONTEXT\_LIMIT # from above
)
`
```
We&rsquo;ll get output looking like this, providing us with *relative results relevance gain* and *discounted cumulative gain (DCG) win rates* at *N* (here, 10).
```
...
`
```
**Relative results relevance gain**: over this test set of 50 generated queries, relevance feedback-based retrieval pulled 5 (44 - 39) more relevant documents from the collection, which the retriever was unable to &ldquo;notice&rdquo; &ndash; a 13% boost over vanilla retrieval.
...
## Conclusion
In this tutorial, we demonstrated how to use relevance feedback on text data to increase the relevance of results in your retrieval pipelines. Here&rsquo;s what to keep in mind:
* **Adaptable to your data** &ndash; works on different modalities, not only text. You can do the same with images, for example.
* **Flexible model choice** &ndash; you can use predefined retrievers and feedback models we support in Cloud Inference and FastEmbed, or define your own, anything, including LLMs and custom LTR models.
* **Built-in evaluation** &ndash; with our framework, you can also evaluate the potential gains of using the Relevance Feedback Query before putting it into production.
*If you&rsquo;d like advice on Relevance Feedback Query usage or have ideas on how to enhance the method, reach out in our [Discord community](https://qdrant.to/discord).*
##### Was this page useful?
Yes
No
Thank you for your feedback! 🙏
We are sorry to hear that. 😔 You can [edit](https:/github.com/qdrant/landing_page/tree/master/qdrant-landing/content/documentation/tutorials-search-engineering/using-relevance-feedback.md) this page on GitHub, or [create](https://github.com/qdrant/landing_page/issues/new/choose) a GitHub issue.
On this page:
* [
View as Markdown](https://qdrant.tech/documentation/tutorials-search-engineering/using-relevance-feedback/index.md)
* [
Edit on Github](https://github.com/qdrant/landing_page/tree/master/qdrant-landing/content/documentation/tutorials-search-engineering/using-relevance-feedback.md)
* [
