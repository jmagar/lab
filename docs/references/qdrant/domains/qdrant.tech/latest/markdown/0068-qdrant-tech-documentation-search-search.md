...
use qdrant\_client::qdrant::{Query, QueryPointsBuilder};
let client = Qdrant::from\_url("http://localhost:6334").build()?;
client
.query(
QueryPointsBuilder::new("{collection\_name}")
.query(Query::new\_nearest(vec![0.2, 0.1, 0.9, 0.7]))
)
.await?;
`
```
...
* Manhattan distance: `Manhattan`\*- [https://en.wikipedia.org/wiki/Taxicab\_geometry](https://en.wikipedia.org/wiki/Taxicab_geometry) *\*Available as of v1.7*
...
## Search API
Let&rsquo;s look at an example of a search query.
...
This parameter may exclude lower or higher scores depending on the used metric. For example, higher scores of Euclidean metric are considered more distant and, therefore, will be excluded.### Payload and vector in the result
...
Search and [recommendation](https://qdrant.tech/documentation/search/explore/#recommendation-api) APIs allow to skip first results of the search and return only the result starting from some specified offset:
Example:
```
`POST /collections/{collection\_name}/points/query
{
...
**Limitations**:
* Only [keyword](https://qdrant.tech/documentation/manage-data/payload/#keyword) and [integer](https://qdrant.tech/documentation/manage-data/payload/#integer) payload values are supported for the `group\_by` parameter. Payload values with other types will be ignored.
* At the moment, pagination is not enabled when using **groups**, so the `offset` parameter is not allowed.### Lookup in groups
...
We are sorry to hear that. 😔 You can [edit](https:/github.com/qdrant/landing_page/tree/master/qdrant-landing/content/documentation/search/search.md) this page on GitHub, or [create](https://github.com/qdrant/landing_page/issues/new/choose) a GitHub issue.
On this page:
* [
View as Markdown](https://qdrant.tech/documentation/search/search/index.md)
* [
Edit on Github](https://github.com/qdrant/landing_page/tree/master/qdrant-landing/content/documentation/search/search.md)
* [
