Troubleshooting: High memory usage when processing many images
...
This is especially noticeable when using `experimental\_download` to process images from URLs, or when sending base64-encoded images in prompts.
...
## [Solution](#solution)
Use the `experimental\_include` option to disable inclusion of request and/or response bodies:
```
`
...
```
### [Options](#options)
The `experimental\_include` option accepts:
