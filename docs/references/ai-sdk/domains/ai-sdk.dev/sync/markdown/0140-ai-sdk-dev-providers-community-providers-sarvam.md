...
headers: {
'api-subscription-key': 'YOUR\_API\_KEY',
},
});
`
```
The `api-subscription-key` needs to be passed in headers. Consider using
`YOUR\_API\_KEY` as environment variables for security.
...
`
```
`language\_code` specifies the language of the input audio and is required for
...
providerOptions: {
sarvam: {
with\_timestamps: true,
},
},
`
```
`with\_timestamps` specifies whether to include start/end timestamps for each
...
providerOptions: {
sarvam: {
with\_diarization: true,
},
},
`
```
`with\_diarization` enables speaker diarization (Beta). • Type: boolean • When
...
`
```
`num\_speakers` sets the number of distinct speakers to detect (only when
`with\_diarization` is true). • Type: number | null • Number of distinct
