...
```
`
OTEL\_EXPORTER\_OTLP\_ENDPOINT=https://api.traceloop.com
OTEL\_EXPORTER\_OTLP\_HEADERS="Authorization=Bearer \<Your API Key\>"
`
```
You can then use the `experimental\_telemetry` option to enable telemetry on supported AI SDK function calls:
```
`
