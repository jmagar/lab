Guides: Build a Custom Memory Tool
...
Copy markdown
# [Build a Custom Memory Tool](#build-a-custom-memory-tool)
...
## [The Storage Primitive: The Filesystem](#the-storage-primitive-the-filesystem)
...
## [The Interface: A Memory Tool](#the-interface-a-memory-tool)
...
### [Structured Actions Tool](#structured-actions-tool)
...
### [Bash-Backed Tool](#bash-backed-tool)
...
## [Step 1: Define the Memory Tool](#step-1-define-the-memory-tool)
...
### [Route A: Structured Actions Tool](#route-a-structured-actions-tool)
...
### [Route B: Bash-Backed Tool](#route-b-bash-backed-tool)
...
### [Appendix: Filesystem Bootstrap](#appendix-filesystem-bootstrap)
...
const MEMORY\_DIR = '.memory';
const MEMORY\_ROOT = resolve(process.cwd(), MEMORY\_DIR);
const CORE\_MEMORY\_PATH = join(MEMORY\_ROOT, 'core.md');
const NOTES\_PATH = join(MEMORY\_ROOT, 'notes.md');
const CONVERSATIONS\_PATH = join(MEMORY\_ROOT, 'conversations.jsonl');
const DEFAULT\_CORE\_MEMORY = `# Core Memory
- Keep this short.
- Put stable user facts here.
`;
const DEFAULT\_NOTES = `# Notes
...
await mkdir(MEMORY\_ROOT, { recursive: true });
await ensureFile(CORE\_MEMORY\_PATH, DEFAULT\_CORE\_MEMORY);
await ensureFile(NOTES\_PATH, DEFAULT\_NOTES);
await ensureFile(CONVERSATIONS\_PATH, '');
}
`
```
...
try {
return await readFile(CORE\_MEMORY\_PATH, 'utf8');
} catch {
return '';
}
}
...
await appendFile(CONVERSATIONS\_PATH, `${JSON.stringify(entry)}\\n`, 'utf8');
}
`
```
### [Appendix: Structured Actions Handler](#appendix-structured-actions-handler)
...
const MEMORY\_FILES = ['core.md', 'notes.md', 'conversations.jsonl'];
...
.replace(/^\\/?\\.memory\\/?/, '')
.replace(/^\\/+/, '');
if (!MEMORY\_FILES.includes(relativePath)) {
throw new Error(`Unsupported memory path: ${path}`);
}
return join(MEMORY\_ROOT, relativePath);
}
...
: MEMORY\_FILES.map(f =\> join(MEMORY\_ROOT, f));
...
matches.push(`${relative(MEMORY\_ROOT, filePath)}:${i + 1}:${line}`);
}
}
}
...
`
```
### [Appendix: Command Guard](#appendix-command-guard)
