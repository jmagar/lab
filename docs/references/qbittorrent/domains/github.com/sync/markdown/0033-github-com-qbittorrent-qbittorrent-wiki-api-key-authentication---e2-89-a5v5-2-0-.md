API Key Authentication (≥v5.2.0) · qbittorrent/qBittorrent Wiki · GitHub
...
Thomas Piccirello edited this page Feb 8, 2026
&middot;
[1 revision](/qbittorrent/qBittorrent/wiki/API-Key-Authentication-(≥v5.2.0)/_history)
Note
...
## Format
[](#format)
API keys are 32 characters long. They start with the prefix `qbt\_` followed by 28 random alphanumeric characters. Each key is generated with 160 bits of entropy.
## Usage
[](#usage)
Specify the key in the `Authorization` header using a scheme of `Bearer`. For example: `Authorization: Bearer \<API\_KEY\>`.
## Generate
[](#generate)
