Compilation FreeBSD (no GUI) · qbittorrent/qBittorrent Wiki · GitHub
...
&middot;
[2 revisions](/qbittorrent/qBittorrent/wiki/Compilation-FreeBSD-(no-GUI)/_history)
...
```
>
1. (Optionally) you can install qBittorrent in the system PATH (root privileges required)
***You can also use `qbittorrent-nox` binary directly without installation (`./src/qbittorrent-nox`)***
>
```
> make install
```
