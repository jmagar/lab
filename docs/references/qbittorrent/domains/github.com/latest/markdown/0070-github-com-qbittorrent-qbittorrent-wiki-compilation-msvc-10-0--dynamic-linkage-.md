Compilation MSVC 10.0 (dynamic linkage) · qbittorrent/qBittorrent Wiki · GitHub
...
&middot;
[2 revisions](/qbittorrent/qBittorrent/wiki/Compilation-MSVC-10.0-(dynamic-linkage)/_history)
# OUTDATED
[](#outdated)
We are building qBittorrent on Windows with MSVC compiler instead of MinGW. This choice was made because [http://www.mingw.org/wiki/wide\_characters MinGW does not support wide characters]. MSVC compiler also generates smaller executables than MinGW.
...
* Download [http://downloads.sourceforge.net/project/boost/boost-jam/3.1.18/boost-jam-3.1.18-1-ntx86.zip?use\_mirror=freefr Boost Jam] (bjam) and [http://sourceforge.net/projects/boost/files/boost/1.51.0/boost\_1\_51\_0.zip/download Boost library] (v1.42-1.51 are known to work) and unzip them
...
* Define ''BOOST\_ROOT'' environment variable and point to the boost source folder (you will need to close and reopen your terminal window after that)
* Download [http://www.slproweb.com/download/Win32OpenSSL-0\_9\_8x.exe OpenSSL v0.9.x] and install it
...
bjam -q --without-python --toolset=msvc variant=release link=static runtime-link=shared logging=none geoip=static dht-support=on boost=source define="BOOST\_FILESYSTEM\_VERSION=2" encryption=openssl
...
\*\* boost libs are generated in ''boost\_1\_51\\stage\\lib''
