Plex API: Stop Backup Database Task - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
There are a number of tasks that can be scheduled to run on a Plex server. This API command will stop the [backup database](/plex-media-server/general/backup/) task.
## URL
```
DELETE http://{ip\_address}:32400/butler/BackupDatabase?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
## Response
No response is returned by the command. The status code will indicate whether the backup database task was stopped successfully.
## Examples
curl Python Powershell
```
curl -X DELETE http://{ip\_address}:32400/butler/BackupDatabase?X-Plex-Token={plex\_token}
```
```
import requests
...
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/butler/BackupDatabase?X-Plex-Token={plex\_token}' -Method DELETE
