Plex API: Get Activities on a Plex Server - Plexopedia
...
## URL
```
GET http://{ip\_address}:32400/activites/?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
Activity Attributes|Attribute|Description|
|uuid|Unique identifier for the activity.|
|type|The type of activity that is running.|
|cancellable|Flag indicating the activity can be cancelled.
...
```
curl -X GET http://{ip\_address}:32400/activites/?X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/activites/?X-Plex-Token={plex\_token}
response = requests.get(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/activites/?X-Plex-Token={plex\_token}' -Method GET
