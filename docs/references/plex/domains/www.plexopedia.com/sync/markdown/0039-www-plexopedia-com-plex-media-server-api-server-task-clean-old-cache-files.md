Plex API: Run Clean Old Cache Files Task - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
There are a number of tasks that can be scheduled to run on a Plex server. This API command will run the [clean old cache files](/plex-media-server/general/clear-cache/) task.
## URL
```
POST http://{ip\_address}:32400/butler/CleanOldCacheFiles?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
## Response
No response is returned by the command. The status code will indicate whether the clean old cache files task was started successfully.
## Remarks
...
```
curl -X POST http://{ip\_address}:32400/butler/CleanOldCacheFiles?X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/butler/CleanOldCacheFiles?X-Plex-Token={plex\_token}
response = requests.post(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/butler/CleanOldCacheFiles?X-Plex-Token={plex\_token}' -Method POST
