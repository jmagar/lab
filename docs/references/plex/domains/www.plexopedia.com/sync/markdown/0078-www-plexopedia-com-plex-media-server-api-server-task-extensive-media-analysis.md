Plex API: Run Extensive Media Analysis Task - Plexopedia
...
## URL
```
POST http://{ip\_address}:32400/butler/DeepMediaAnalysis?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
```
curl -X POST http://{ip\_address}:32400/butler/DeepMediaAnalysis?X-Plex-Token={plex\_token}
```
```
import requests
...
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/butler/DeepMediaAnalysis?X-Plex-Token={plex\_token}' -Method POST
