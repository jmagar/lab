Plex API: Terminate a Transcode Session - Plexopedia
...
## URL
```
DELETE http://{ip\_address}:32400{key}?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
|key|The key associated with a session. This key can be found by calling the [Active Sessions API command](/plex-media-server/api/server/sessions/) and looking for the `key` attribute in the `TranscodeSession` element.|
## Return Status
...
## Remarks
The `key` attribute in the [Active Sessions API command](/plex-media-server/api/server/sessions/) will be in the format `/transcode/session/{id}`. This entire key will need to be used in the API request.
## Examples
curl Python Powershell
```
curl -X DELETE http://{ip\_address}:32400{key}?X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400{key}?X-Plex-Token={plex\_token}
response = requests.delete(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400{key}?X-Plex-Token={plex\_token}' -Method DELETE
