Plex API: Delete a Playlist - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
Each user on a Plex server can [create their own playlists](/plex-media-server/general/create-playlist/). A playlist can contain either videos, music, or photos, and can't be shared with other users. There are smart and non-smart playlists. This API command will delete a playlist for a user.
## URL
```
DELETE http://{ip\_address}:32400/playlists/{id}?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
```
curl -X DELETE http://{ip\_address}:32400/playlists/{id}?X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/playlists/{id}?X-Plex-Token={plex\_token}
response = requests.delete(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/playlists/{id}?X-Plex-Token={plex\_token}' -Method DELETE
