Plex API: Run All Scheduled Tasks - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
There are a number of tasks that can be scheduled to run on a Plex server. This API command will run all the scheduled tasks.
## URL
```
POST http://{ip\_address}:32400/butler/?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
|200|Success - The tasks have started successfully.|
|401|Unauthorized - The Plex token provided was not valid.|
## Response
No response is returned by the command. The status code will indicate whether the scheduled tasks were started successfully.
## Remarks
...
```
curl -X POST http://{ip\_address}:32400/butler/?X-Plex-Token={plex\_token}
```
```
import requests
...
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/butler/?X-Plex-Token={plex\_token}' -Method POST
