Plex API: Get Update Status - Plexopedia
...
## URL
```
GET http://{ip\_address}:32400/updater/status?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
|checkedAt|The date and time in [epoch time](https://en.wikipedia.org/wiki/Unix_time), when an update was last checked.|
...
Release Attributes|Attribute|Description|
...
## Remarks
The Plex Media Server update file can be found in the `Updates` folder in the [Plex data directory](/plex-media-server/general/data-directory/).
## Examples
curl Python Powershell
```
curl -X GET http://{ip\_address}:32400/updater/status?X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/updater/status?X-Plex-Token={plex\_token}
response = requests.get(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/updater/status?X-Plex-Token={plex\_token}' -Method GET
