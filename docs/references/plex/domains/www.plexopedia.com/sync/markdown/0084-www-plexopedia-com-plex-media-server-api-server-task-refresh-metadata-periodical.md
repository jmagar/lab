Plex API: Stop Refresh Metadata Periodically Task - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
There are a number of tasks that can be scheduled to run on a Plex server. This API command will stop the refresh local metadata task.
## URL
```
DELETE http://{ip\_address}:32400/butler/RefreshPeriodicMetadata?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
## Response
No response is returned by the command. The status code will indicate whether the refresh metadata periodically task was stopped successfully.
## Examples
curl Python Powershell
```
curl -X DELETE http://{ip\_address}:32400/butler/RefreshPeriodicMetadata?X-Plex-Token={plex\_token}
```
```
import requests
...
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/butler/RefreshPeriodicMetadata?X-Plex-Token={plex\_token}' -Method DELETE
