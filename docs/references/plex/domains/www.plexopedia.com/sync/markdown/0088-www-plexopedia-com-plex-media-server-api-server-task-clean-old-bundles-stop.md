Plex API: Stop Clean Old Bundles Task - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
There are a number of tasks that can be scheduled to run on a Plex server. This API command will stop the [clean old bundles](/plex-media-server/general/clean-bundles/) task.
## URL
```
DELETE http://{ip\_address}:32400/butler/CleanOldBundles?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
```
curl -X DELETE http://{ip\_address}:32400/butler/CleanOldBundles?X-Plex-Token={plex\_token}
```
```
import requests
...
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/butler/CleanOldBundles?X-Plex-Token={plex\_token}' -Method DELETE
