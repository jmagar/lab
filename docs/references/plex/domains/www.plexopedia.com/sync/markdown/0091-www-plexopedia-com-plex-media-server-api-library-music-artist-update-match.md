Plex API: Update Music Artist Details Using Match - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
You can update metadata for a music artist using Plex's match feature and the API. This API command will use the information returned from the [search for match API](/plex-media-server/api/library/search-match/) to update the artist's metadata.
## URL
```
PUT http://{ip\_address}:32400/library/metadata/{id}/match?guid={guid}&name={name}&X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
|id|The key associated with the artist to be updated. This key can be found by calling the [Get All Music Artists API command](/plex-media-server/api/library/music/) and looking for the music artist to be updated.|
...
|404|Not Found - The `Music Artist ID` value is incorrect.|
## Response
...
|size|The number of music artists.|
...
```
curl -X PUT http://{ip\_address}:32400/library/metadata/{id}/match?guid={guid}&name={name}&X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/library/metadata/{id}/match?guid={guid}&name={name}&X-Plex-Token={plex\_token}
response = requests.put(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/library/metadata/{id}/match?guid={guid}&name={name}&X-Plex-Token={plex\_token}' -Method PUT
