Plex API: Delete an Item from a Playlist - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
Each user on a Plex server can [create their own playlists](/plex-media-server/general/create-playlist/). A playlist can contain either videos, music, or photos, and can't be shared with other users. There are smart and non-smart playlists. This API command will delete a single item from a non-smart playlist.
## URL
```
DELETE http://{ip\_address}:32400/playlists/{id}/items/{item\_id}?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
|item\_id|The ID associated with the media item in the playlist. Can be found by calling the [View Items in a Playlist](/plex-media-server/api/playlists/view-items/) command and getting the `playlistItemID` attribute value.|
## Return Status
...
The value is easy to find in the URL of the item. It has the structure `/library/metadata/{item\_id}`. You will use the entire value of the `key` parameter from the media item URL.
## Examples
curl Python Powershell
```
curl -X DELETE http://{ip\_address}:32400/playlists/{id}/items/{item\_id}?X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/playlists/{id}/items/{item\_id}?X-Plex-Token={plex\_token}
response = requests.delete(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/playlists/{id}/items/{item\_id}?X-Plex-Token={plex\_token}' -Method DELETE
