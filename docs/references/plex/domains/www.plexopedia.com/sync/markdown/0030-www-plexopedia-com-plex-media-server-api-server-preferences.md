Plex API: Get Server Preferences - Plexopedia
...
## URL
```
GET http://{ip\_address}:32400/:/prefs?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
## Response
XML string value that lists the [preferences that are available in the Plex server](/plex-media-server/general/hidden-settings/). An example of the XML returned from the request is shown below:
```
...
Setting Attributes|Attribute|Description|
...
You can use the settings returned from this endpoint as inputs into the [Set a Server Preference](/plex-media-server/api/server/preference-set/) endpoint to transfer settings between Plex servers.
## Examples
curl Python Powershell
```
curl -X GET http://{ip\_address}:32400/:/prefs?X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/:/prefs?X-Plex-Token={plex\_token}
response = requests.get(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/:/prefs?X-Plex-Token={plex\_token}' -Method GET
