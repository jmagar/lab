Plex API: Run Refresh Metadata Periodically Task - Plexopedia
...
## URL
```
POST http://{ip\_address}:32400/butler/RefreshPeriodicMetadata?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
## Response
No response is returned by the command. The status code will indicate whether the refresh metadata periodically task was started successfully.
## Remarks
This task will update information about [music artists](/plex-media-server/general/add-music/) and [TV shows](/plex-media-server/general/add-tv-shows-plex/). This ensures that artists have accurate tour date details and [Plex Pass](/plex-media-server/general/plex-pass/) users with music libraries will automatically receive new lyrics.
This task will only [refresh metadata](/plex-media-server/general/refresh-metadata/) for music and TV shows.
...
* The task will run immediately if it is outside the configured window.## Examples
curl Python Powershell
```
curl -X POST http://{ip\_address}:32400/butler/RefreshPeriodicMetadata?X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/butler/RefreshPeriodicMetadata?X-Plex-Token={plex\_token}
response = requests.post(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/butler/RefreshPeriodicMetadata?X-Plex-Token={plex\_token}' -Method POST
