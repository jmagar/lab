Plex API: Stop Extensive Media Analysis Task - Plexopedia
...
## URL
```
DELETE http://{ip\_address}:32400/butler/DeepMediaAnalysis?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
```
curl -X DELETE http://{ip\_address}:32400/butler/DeepMediaAnalysis?X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/butler/DeepMediaAnalysis?X-Plex-Token={plex\_token}
response = requests.delete(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/butler/DeepMediaAnalysis?X-Plex-Token={plex\_token}' -Method DELETE
