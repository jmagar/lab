Plex API: Run Clean Old Bundles Task - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
There are a number of tasks that can be scheduled to run on a Plex server. This API command will run the [clean old bundles](/plex-media-server/general/clean-bundles/) task.
## URL
```
POST http://{ip\_address}:32400/butler/CleanOldBundles?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
If you don't want to run the scheduled task, you can call the [clean bundles API](/plex-media-server/api/library/clean-bundles/) directly and clean the bundles on demand.
## Examples
curl Python Powershell
```
curl -X POST http://{ip\_address}:32400/butler/CleanOldBundles?X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/butler/CleanOldBundles?X-Plex-Token={plex\_token}
response = requests.post(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/butler/CleanOldBundles?X-Plex-Token={plex\_token}' -Method POST
