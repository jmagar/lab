Plex API: Stop an Activity on a Plex Server - Plexopedia
...
## URL
```
DELETE http://{ip\_address}:32400/activites/{id}?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
|id|The UUID associated with a running activity. This key can be found by calling the [Get All Activities API command](/plex-media-server/api/server/activities/).|
## Return Status
|HTTP Code|Description|
|200|Success - The activity was canceled.|
|401|Unauthorized - The Plex token provided was not valid.|
|404|Not Found - The activity for the provided UUID was not running.|
## Response
...
```
curl -X DELETE http://{ip\_address}:32400/activites/{id}?X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/activites/{id}?X-Plex-Token={plex\_token}
response = requests.delete(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/activites/{id}?X-Plex-Token={plex\_token}' -Method DELETE
