Plex API: Update a TV Show Series Using Match - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
You can update metadata for a TV show series using Plex's match feature and the API. This API command will use the information returned from the [search for match API](/plex-media-server/api/library/search-match/) to update the series' metadata.
## URL
```
...
|id|The key associated with the TV show to be updated. This key can be found by calling the [Get All TV Shows API command](/plex-media-server/api/library/tvshows/) and looking for the TV show series to be updated.|
|guid|This is the unique identifier returned by the [search for match API](/plex-media-server/api/library/search-match/). Each `SearchResult` will have a `guid` attribute. It will be in the format `plex://show/{string\_value}`. That GUID string is what will be used for this parameter value.|
