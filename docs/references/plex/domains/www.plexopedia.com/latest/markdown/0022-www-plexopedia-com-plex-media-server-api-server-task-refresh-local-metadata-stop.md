Plex API: Stop Refresh Local Metadata Task - Plexopedia
...
## URL
```
DELETE http://{ip\_address}:32400/butler/RefreshLocalMedia?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
```
curl -X DELETE http://{ip\_address}:32400/butler/RefreshLocalMedia?X-Plex-Token={plex\_token}
```
```
import requests
...
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/butler/RefreshLocalMedia?X-Plex-Token={plex\_token}' -Method DELETE
