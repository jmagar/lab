Plex API: Scan a Single Library - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
When you add or remove media from your Plex server you will need to [scan the libraries](/plex-media-server/general/scan-library/) to update Plex with the changed media. You can run the scan from the Plex Web app, or you can use this API command to run the scan from outside of Plex.
...
## URL
```
GET http://{ip\_address}:32400/library/sections/{id}/refresh?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
If you only need to scan for a few media items, you can use the [Scan Partial Library API command](/plex-media-server/api/library/scan-partial/).
## Examples
curl Python Powershell
```
curl -X GET http://{ip\_address}:32400/library/sections/{id}/refresh?X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/library/sections/{id}/refresh?X-Plex-Token={plex\_token}
response = requests.get(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/library/sections/{id}/refresh?X-Plex-Token={plex\_token}' -Method GET
