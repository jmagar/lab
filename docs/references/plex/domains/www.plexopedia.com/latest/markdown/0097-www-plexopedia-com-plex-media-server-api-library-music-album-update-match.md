Plex API: Update Music Album Details Using Match - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
You can update metadata for a music album using Plex's match feature and the API. This API command will use the information returned from the [search for match API](/plex-media-server/api/library/search-match/) to update the album's metadata.
## URL
```
PUT http://{ip\_address}:32400/library/metadata/{id}/match?guid={guid}&name={name}&year={year}&{tracks}&X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
|id|The key associated with the album to be updated. This key can be found by calling the [Get Music Albums for an Artist API command](/plex-media-server/api/library/music-albums-artist/) and using the music album `ratingKey`.|
|guid|This is the unique identifier returned by the [search for match API](/plex-media-server/api/library/search-match/). Each `SearchResult` will have a `guid` attribute. It will be in the format `mbid://{string\_value}`. That GUID string is what will be used for this parameter value.|
...
Each track is one element in an array, with the index of the array being the ID for that track. The ID for the track can get found by calling the [search for match API](/plex-media-server/api/library/search-match/) for the album.
...
```
curl -X PUT http://{ip\_address}:32400/library/metadata/{id}/match?guid={guid}&name={name}&year={year}&{tracks}&X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/library/metadata/{id}/match?guid={guid}&name={name}&year={year}&{tracks}&X-Plex-Token={plex\_token}
response = requests.put(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/library/metadata/{id}/match?guid={guid}&name={name}&year={year}&{tracks}&X-Plex-Token={plex\_token}' -Method PUT
