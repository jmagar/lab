Plex API: Log Multiple Messages - Plexopedia
...
## URL
```
POST http://{ip\_address}:32400/log?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
The parameters in the body are the same as those specified when [logging a single message to the log](/plex-media-server/api/server/log-single/).
...
```
curl -X POST http://{ip\_address}:32400/log?X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/log?X-Plex-Token={plex\_token}
response = requests.post(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/log?X-Plex-Token={plex\_token}' -Method POST
...
### Troubleshooting
[Log a Single Message](https://www.plexopedia.com/plex-media-server/api/server/log-single/) Log Multiple Messages [Download Databases](https://www.plexopedia.com/plex-media-server/api/server/download-databases/) [Download Logs](https://www.plexopedia.com/plex-media-server/api/server/download-logs/)
### Reference
