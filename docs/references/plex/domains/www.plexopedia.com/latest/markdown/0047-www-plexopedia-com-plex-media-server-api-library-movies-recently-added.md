Plex API: Get Recently Added Movies - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
This API command is used to get all recently added movies to a library.
## URL
```
...
\</MediaContainer\>
```
The XML returned provides a list of the all movies in a library that have been recently added to the Plex server. The root is the `MediaContainer` element. This element contains a few attributes that provide overall information about the movies on the server.
