Plex API: Run Optimize Database Task - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
There are a number of tasks that can be scheduled to run on a Plex server. This API command will run the [optimize database](/plex-media-server/general/optimize-database/) task.
## URL
```
POST http://{ip\_address}:32400/butler/OptimizeDatabase?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
## Response
No response is returned by the command. The status code will indicate whether the [optimize database](/plex-media-server/general/optimize-database/) task was started successfully.
## Remarks
...
You can also use the [Optimize Database API command](/plex-media-server/api/library/optimize-database/) to optimize the database outside of a scheduled task.
## Examples
curl Python Powershell
```
curl -X POST http://{ip\_address}:32400/butler/OptimizeDatabase?X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/butler/OptimizeDatabase?X-Plex-Token={plex\_token}
response = requests.post(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/butler/OptimizeDatabase?X-Plex-Token={plex\_token}' -Method POST
