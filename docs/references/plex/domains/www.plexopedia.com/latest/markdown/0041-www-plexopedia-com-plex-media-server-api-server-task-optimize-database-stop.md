Plex API: Stop Optimize Database Task - Plexopedia
[ Buy me a coffee](https://buymeacoffee.com/plexopedia)
There are a number of tasks that can be scheduled to run on a Plex server. This API command will stop the [optimize database](/plex-media-server/general/optimize-database/) task.
## URL
```
DELETE http://{ip\_address}:32400/butler/OptimizeDatabase?X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
## Response
No response is returned by the command. The status code will indicate whether the [optimize database](/plex-media-server/general/optimize-database/) task was stopped successfully.
## Examples
curl Python Powershell
```
curl -X DELETE http://{ip\_address}:32400/butler/OptimizeDatabase?X-Plex-Token={plex\_token}
```
```
import requests
...
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/butler/OptimizeDatabase?X-Plex-Token={plex\_token}' -Method DELETE
