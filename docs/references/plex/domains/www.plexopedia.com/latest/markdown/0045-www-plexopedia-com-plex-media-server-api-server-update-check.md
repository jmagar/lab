Plex API: Check for Updates - Plexopedia
...
## URL
```
PUT http://{ip\_address}:32400/updater/check?download={download}&X-Plex-Token={plex\_token}
```
### Parameters
|Name|Description|
...
## Remarks
When an update is downloaded, it is saved in the `Updates` folder in the [Plex data directory](/plex-media-server/general/data-directory/).
## Examples
curl Python Powershell
```
curl -X PUT http://{ip\_address}:32400/updater/check?download={download}&X-Plex-Token={plex\_token}
```
```
import requests
plex\_url = http://{ip\_address}:32400/updater/check?download={download}&X-Plex-Token={plex\_token}
response = requests.put(plex\_url)
print(response.text)
```
```
$response = Invoke-RestMethod 'http://{ip\_address}:32400/updater/check?download={download}&X-Plex-Token={plex\_token}' -Method PUT
