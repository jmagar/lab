Migrate from a legacy VPN to Tailscale · Tailscale Docs
...
When you add [access rules for the advertised subnet routes](/docs/features/subnet-routers?tab=linux#add-access-rules-for-the-advertised-subnet-routes), in Tailscale you grant access to the routes themselves (such as `192.0.2.0/24`). This is different from some VPNs, which grant routing access to a device, not to the routes that the device provides.
