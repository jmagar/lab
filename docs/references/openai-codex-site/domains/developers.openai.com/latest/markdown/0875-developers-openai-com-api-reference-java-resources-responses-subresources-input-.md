...
Optional\<[Order](</api/reference/java/resources/responses/subresources/input_items/methods/list#(resource) responses.input_items > (method) list > (params) default > (param) order > (schema)>)\> order
...
InputItemListPage page = client.responses().inputItems().list("response\_id");
}
}`
```
```
