...
[](<#(resource) admin.organization.projects.users > (method) update > (params) default > (param) role > (schema)>)
...
project\_user = openai.admin.organization.projects.users.update("user\_id", project\_id: "project\_id", role: :owner)
puts(project\_user)`
```
```
`{
