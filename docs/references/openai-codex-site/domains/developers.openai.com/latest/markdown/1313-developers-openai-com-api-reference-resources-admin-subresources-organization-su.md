...
[](<#(resource) admin.organization.projects.users.roles > (method) list > (network schema) > (property) data>)
has\_more: boolean
Whether additional assignments are available when paginating.
[](<#(resource) admin.organization.projects.users.roles > (method) list > (network schema) > (property) has_more>)
next: string
Cursor to fetch the next page of results, or `null` when there are no more assignments.
[](<#(resource) admin.organization.projects.users.roles > (method) list > (network schema) > (property) next>)
object: "list"
Always `list`.
[](<#(resource) admin.organization.projects.users.roles > (method) list > (network schema) > (property) object>)
### List project user role assignments
HTTP
HTTP
HTTP
...
Terraform
```
`curl https://api.openai.com/v1/projects/proj\_abc123/users/user\_abc123/roles \\
