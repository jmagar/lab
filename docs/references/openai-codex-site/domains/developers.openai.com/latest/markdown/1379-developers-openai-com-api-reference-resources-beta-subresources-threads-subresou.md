...
data: array of [RunStep](</api/reference/resources/beta#(resource) beta.threads.runs.steps > (model) run_step > (schema)>) { id, assistant\_id, cancelled\_at, 13 more }
id: string
...
[](<#(resource) beta.threads.runs.steps > (method) list > (network schema) > (property) data>)
first\_id: string
[](<#(resource) beta.threads.runs.steps > (method) list > (network schema) > (property) first_id>)
has\_more: boolean
[](<#(resource) beta.threads.runs.steps > (method) list > (network schema) > (property) has_more>)
last\_id: string
[](<#(resource) beta.threads.runs.steps > (method) list > (network schema) > (property) last_id>)
object: string
[](<#(resource) beta.threads.runs.steps > (method) list > (network schema) > (property) object>)
### List run steps
HTTP
HTTP
HTTP
...
Terraform
```
`curl https://api.openai.com/v1/threads/thread\_abc123/runs/run\_abc123/steps \\
