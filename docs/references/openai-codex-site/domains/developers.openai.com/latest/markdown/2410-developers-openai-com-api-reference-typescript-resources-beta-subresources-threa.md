...
[](<#(resource) beta.threads.runs.steps > (method) list > (params) default > (param) thread_id>)
after?: string
...
order?: "asc" | "desc"
Query param: Sort order by the `created\_at` timestamp of the objects. `asc` for ascending order and `desc` for descending order.
