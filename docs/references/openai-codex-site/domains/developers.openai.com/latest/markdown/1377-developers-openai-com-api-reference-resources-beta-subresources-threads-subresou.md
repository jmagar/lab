...
[](<#(resource) beta.threads.runs > (method) submit_tool_outputs > (params) 0.non_streaming > (param) tool_outputs > (schema) > (items) > (property) output>)
...
[](<#(resource) beta.threads.runs > (method) submit_tool_outputs > (params) 0.non_streaming > (param) tool_outputs > (schema) > (items) > (property) tool_call_id>)
[](<#(resource) beta.threads.runs > (method) submit_tool_outputs > (params) 0.non_streaming > (param) tool_outputs > (schema)>)
stream: optional boolean
If `true`, returns a stream of events that happen during the Run as server-sent events, terminating when the Run enters a terminal state with a `data: [DONE]` message.
[](<#(resource) beta.threads.runs > (method) submit_tool_outputs > (params) 0.non_streaming > (param) stream > (schema)>)
...
Terraform
```
`curl https://api.openai.com/v1/threads/thread\_123/runs/run\_123/submit\_tool\_outputs \\
