...
[](<#(resource) chat.completions > (method) update > (params) default > (param) metadata > (schema)>)
...
updated\_completion = client.chat.completions.update(completion\_id=first\_id, request\_body={"metadata": {"foo": "bar"}})
print(updated\_completion)
`
```
```
`{
