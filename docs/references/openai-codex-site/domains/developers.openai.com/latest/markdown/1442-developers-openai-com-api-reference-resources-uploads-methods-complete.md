...
[](<#(resource) uploads > (method) complete > (params) 0 > (param) part_ids > (schema)>)
md5: optional string
The optional md5 checksum for the file contents to verify if the bytes uploaded matches what you expect.
[](<#(resource) uploads > (method) complete > (params) 0 > (param) md5 > (schema)>)
...
Terraform
```
`curl https://api.openai.com/v1/uploads/upload\_abc123/complete
-d '{
"part\_ids": ["part\_def456", "part\_ghi789"]
}'
`
```
```
