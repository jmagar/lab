...
[](<#(resource) admin.organization.projects.roles > (method) list > (network schema) > (property) data>)
has\_more: boolean
Whether more roles are available when paginating.
[](<#(resource) admin.organization.projects.roles > (method) list > (network schema) > (property) has_more>)
next: string
Cursor to fetch the next page of results, or `null` when there are no additional roles.
[](<#(resource) admin.organization.projects.roles > (method) list > (network schema) > (property) next>)
object: "list"
Always `list`.
[](<#(resource) admin.organization.projects.roles > (method) list > (network schema) > (property) object>)
### List project roles
HTTP
HTTP
HTTP
...
Terraform
```
`curl https://api.openai.com/v1/projects/proj\_abc123/roles?limit=20 \\
