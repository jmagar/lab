...
Oct 29, 2025
# User guide for gpt-oss-safeguard
[ RO ](https://roost.tools)
[ ROOST ](https://roost.tools)
[
View on GitHub
](https://github.com/openai/openai-cookbook/blob/main/articles/gpt-oss-safeguard-guide.md) [
Download raw
](https://raw.githubusercontent.com/openai/openai-cookbook/main/articles/gpt-oss-safeguard-guide.md)
...
Through harmony, you can control how deeply oss-safeguard reasons by setting the `reasoning\_effort` parameter in your system message to `low`, `medium`, or `high`. The model uses `medium` by default if it is not set. Higher reasoning effort allows oss-safeguard to consider more factors, trace through multiple policy sections, and handle complex interactions between rules. Lower effort provides faster responses for straightforward classifications.
...
{"violation": 1, "policy\_category": "\<category\_or\_rule\_id\>"}
If the content does NOT violate this policy, return:
{"violation": 0, "policy\_category": null}
Example:
{"violation": 1, "policy\_category": "H2.f"}`
```
### Including Rationale
