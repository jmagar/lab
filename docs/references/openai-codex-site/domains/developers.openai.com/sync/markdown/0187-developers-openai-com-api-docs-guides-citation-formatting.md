...
### Template
```
`{CITATION\_START}\<citation\_family\>{CITATION\_DELIMITER}\<source\_id\>{CITATION\_DELIMITER}\<locator\>{CITATION\_STOP}`
```
### Example
```
...
12
13
`\<extra\_considerations\_for\_citations\>
...
`${CITATION\_START}(?\<family\>${familyPattern})${CITATION\_DELIMITER}(?\<body\>[\\\\s\\\\S]\*?)${CITATION\_STOP}`,
"g"
);
/\*\* @type {Citation[]} \*/
const citations = [];
...
A citation to a single source must be written as:
{CITATION\_START}cite{CITATION\_DELIMITER}\<block\_id\>{CITATION\_STOP}
For example:
