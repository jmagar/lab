...
maxLength32000
minLength1
[](<#(resource) videos > (method) remix > (params) default > (param) prompt > (schema)>)
...
video = openai.videos.remix("video\_123", prompt: "Extend the scene with the cat taking a bow to the cheering audience")
puts(video)
`
```
```
