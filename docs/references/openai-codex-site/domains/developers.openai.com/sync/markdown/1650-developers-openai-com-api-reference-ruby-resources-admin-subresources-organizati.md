...
[](<#(resource) admin.organization.certificates > (method) deactivate > (params) default > (param) certificate_ids > (schema)>)
...
page = openai.admin.organization.certificates.deactivate(certificate\_ids: ["cert\_abc"])
puts(page)`
```
```
`{
