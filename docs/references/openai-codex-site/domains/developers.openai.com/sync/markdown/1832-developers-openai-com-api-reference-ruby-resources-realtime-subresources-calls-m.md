...
[](<#(resource) realtime.calls > (method) accept > (params) default > (param) tool_choice > (schema) + (resource) responses > (model) tool_choice_options > (schema)>)
...
result = openai.realtime.calls.accept("call\_id", type: :realtime)
puts(result)`
```
##### Returns Examples