...
[](<#(resource) beta.threads.messages > (method) update > (params) default > (param) metadata > (schema)>)
...
message = openai.beta.threads.messages.update("message\_id", thread\_id: "thread\_id")
puts(message)`
```
```
`{
