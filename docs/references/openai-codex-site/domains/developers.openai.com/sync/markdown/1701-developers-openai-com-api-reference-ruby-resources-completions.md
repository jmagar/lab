...
completions.create(\*\*kwargs) -\> [Completion](</api/reference/ruby/resources/completions#(resource) completions > (model) completion > (schema)>) { id, choices, created, 4 more }
POST/completions
...
choices: Array[[CompletionChoice](</api/reference/ruby/resources/completions#(resource) completions > (model) completion_choice > (schema)>) { finish\_reason, index, logprobs, text } ]
