...
Optional\<[Order](</api/reference/java/resources/beta/subresources/threads/subresources/messages/methods/list#(resource) beta.threads.messages > (method) list > (params) default > (param) order > (schema)>)\> order
...
MessageListPage page = client.beta().threads().messages().list("thread\_id");
}
}`
```
```
