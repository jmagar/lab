...
ModerationMultiModalInput = [ModerationImageURLInput](</api/reference/ruby/resources/moderations#(resource) moderations > (model) moderation_image_url_input > (schema)>) { image\_url, type } | [ModerationTextInput](</api/reference/ruby/resources/moderations#(resource) moderations > (model) moderation_text_input > (schema)>) { text, type }
An object describing an image to classify.
One of the following:
class ModerationImageURLInput { image\_url, type }
An object describing an image to classify.
