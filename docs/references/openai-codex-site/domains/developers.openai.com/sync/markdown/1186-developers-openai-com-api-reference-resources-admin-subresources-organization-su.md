...
data: array of [ProjectAPIKey](</api/reference/resources/admin#(resource) admin.organization.projects.api_keys > (model) project_api_key > (schema)>) { id, created\_at, last\_used\_at, 4 more }
id: string
...
[](<#(resource) admin.organization.projects.api_keys > (method) list > (network schema) > (property) data>)
first\_id: string
[](<#(resource) admin.organization.projects.api_keys > (method) list > (network schema) > (property) first_id>)
has\_more: boolean
[](<#(resource) admin.organization.projects.api_keys > (method) list > (network schema) > (property) has_more>)
last\_id: string
[](<#(resource) admin.organization.projects.api_keys > (method) list > (network schema) > (property) last_id>)
object: "list"
[](<#(resource) admin.organization.projects.api_keys > (method) list > (network schema) > (property) object>)
### List project API keys
HTTP
HTTP
HTTP
...
Terraform
```
`curl https://api.openai.com/v1/organization/projects/proj\_abc/api\_keys?after=key\_abc&limit=20 \\
