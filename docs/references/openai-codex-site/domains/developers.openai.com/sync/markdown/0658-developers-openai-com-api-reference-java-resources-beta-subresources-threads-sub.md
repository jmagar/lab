...
Optional\<[Order](</api/reference/java/resources/beta/subresources/threads/subresources/runs/methods/list#(resource) beta.threads.runs > (method) list > (params) default > (param) order > (schema)>)\> order
...
RunListPage page = client.beta().threads().runs().list("thread\_id");
}
}`
```
```
