Cancel vector store file batch | OpenAI API Reference
...
[](<#(resource) vector_stores.file_batches > (method) cancel > (params) default > (param) vector_store_id > (schema)>)
batch\_id: str
[](<#(resource) vector_stores.file_batches > (method) cancel > (params) default > (param) batch_id > (schema)>)
...
client = OpenAI()
deleted\_vector\_store\_file\_batch = client.vector\_stores.file\_batches.cancel(
vector\_store\_id="vs\_abc123",
file\_batch\_id="vsfb\_abc123"
)
print(deleted\_vector\_store\_file\_batch)
`
```
```
`{
