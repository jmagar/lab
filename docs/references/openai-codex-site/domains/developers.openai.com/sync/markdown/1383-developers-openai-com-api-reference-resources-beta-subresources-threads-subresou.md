Retrieve run | OpenAI API Reference
...
thread\_id: string
[](<#(resource) beta.threads.runs > (method) retrieve > (params) default > (param) thread_id > (schema)>)
run\_id: string
[](<#(resource) beta.threads.runs > (method) retrieve > (params) default > (param) run_id > (schema)>)
...
Terraform
```
`curl https://api.openai.com/v1/threads/thread\_abc123/runs/run\_abc123 \\
