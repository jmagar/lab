...
[](<#(resource) admin.organization.admin_api_keys > (method) retrieve > (params) default>)
...
AdminApiKey adminApiKey = client.admin().organization().adminApiKeys().retrieve("key\_id");
}
}`
```
```
