...
**OpenAPI spec:** `docs/api-specs/radarr.openapi.json` (164 paths)  
**SDK sub-modules:** `crates/lab-apis/src/radarr/client/` (19 modules, 41 methods)  
**MCP actions:** `crates/lab/src/mcp/services/radarr.rs` (40 actions + built-in `help`)  
...
| ⬜ | Not implemented or not yet live-tested |
| — | Not applicable |

**Column definitions:**
1. **Impl** — SDK client method exists in `lab-apis`
2. **MCP** — Action wired in MCP dispatcher AND verified against live Radarr instance
3. **CLI** — Subcommand wired in CLI AND verified against live Radarr instance
4. **API** — HTTP POST `/v1/radarr` action dispatch verified against live Radarr instance

> ⚠️ **None of the handler columns (MCP / CLI / API) are green yet.** They require manual live testing against a running Radarr instance. Tests passing in CI does not qualify.
...
> **Note:** `manual_import_list()` exists in the SDK but is **not yet wired** in MCP or CLI.
