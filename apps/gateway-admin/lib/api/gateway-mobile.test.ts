...
test('buildGatewayEndpointPreview falls back to in-process service label for lab gateways without url', () => {
  const gateway = {
    ...mockGateways[0],
    name: 'radarr',
  ...
  assert.equal(buildGatewayEndpointPreview(gateway), 'radarr in-process service')
})
